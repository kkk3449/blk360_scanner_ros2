...
LOG="$WS/scripts/last_test/run_mr_compare.log"
CONFIGS=${CONFIGS:-"mr_disk mr_vis"}
...
for NAME in $CONFIGS; do
  MODEL=$([ "$NAME" = mr_disk ] && echo disk || echo visibility)
  log "===== RUN $NAME  (model=$MODEL R=$R stall=$STALL_TIMEOUT multiroom) ====="
  kill_stack
