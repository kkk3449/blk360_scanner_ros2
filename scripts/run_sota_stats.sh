...
OUT="$HOME/blk360_sotastats"
LOG="$WS/scripts/last_test/run_sota_stats.log"
N=${N:-5}
...
for i in $(seq 1 "$N"); do
  NAME="sota_run$i"
  log "===== RUN $i/$N  ($NAME: R=$R stall=$STALL_TIMEOUT completion) ====="
  kill_stack
  ...
  log "launching active_mapping (visibility SOTA only)..."
  nohup ros2 launch blk360_bringup active_mapping.launch.py \
      ...
      coverage_model:=visibility min_new_visible_ratio:=0.30 \
