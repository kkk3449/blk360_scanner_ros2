...
OUT="$HOME/blk360_fieldsim_gz2"
LOG="$WS/scripts/last_test/run_field2.log"
...
log "===== TESTROOM FIELD RUN v2 (new world)  (interval=$INTERVAL R=$R visibility) ====="
kill_stack

log "launching testroom sim (headless)..."
nohup ros2 launch blk360_bringup testroom_sim.launch.py gui:=$GUI world_name:=testroom_field x_pose:=2.03 y_pose:=0.28 >>"$LOG" 2>&1 &
sleep "$SIM_WARMUP"
