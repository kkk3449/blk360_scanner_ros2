...
LOG="$WS/scripts/last_test/run_vis_cc.log"
...
MOCK_SCAN=1.0
MOCK_DL=2.0
STALL_TIMEOUT=60.0     # match the ablation table (R4 = 3 scans) for consistency
RUN_TIMEOUT=1300
SIM_WARMUP=18
...
nohup ros2 launch blk360_bringup active_mapping.launch.py \
    ...
    coverage_model:=visibility min_new_visible_ratio:=0.30 min_new_visible_area_m2:=5.0 coverage_completion:=true min_pocket_area_m2:=0.5 \
...
nohup python3 "$WS/scripts/traj_logger.py" --ros-args \
    -p use_sim_time:=true -p out:="$OUT/traj_R6cc.csv" >>"$LOG" 2>&1 &
TRAJ_PID=$!
...
log "trajectory logged -> $OUT/traj_R6cc.csv ($(wc -l < "$OUT/traj_R6cc.csv" 2>/dev/null || echo 0) points)"
