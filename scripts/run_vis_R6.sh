...
LOG="$WS/scripts/last_test/run_vis_R6.log"
...
nohup python3 "$WS/scripts/traj_logger.py" --ros-args \
    -p use_sim_time:=true -p out:="$OUT/traj_R6.csv" >>"$LOG" 2>&1 &
TRAJ_PID=$!
...
log "trajectory logged -> $OUT/traj_R6.csv ($(wc -l < "$OUT/traj_R6.csv" 2>/dev/null || echo 0) points)"
