...
% CAMERA-READY: first-page-only header (two lines, top-left) per RAAICON 2026
% camera-ready guideline; no header on subsequent pages, no page numbers.
\fancypagestyle{raaiconfirst}{%
  \fancyhf{}%
  \renewcommand{\headrulewidth}{0pt}%
  \fancyhead[L]{\footnotesize\parbox{\textwidth}{%
    2026 IEEE 5th International Conference on Robotics, Automation,
    Artificial-Intelligence and Internet-of-Things (RAAICON)\\
    18--19 September 2026, Jashore University of Science and Technology
    (JUST), Jashore, Bangladesh}}%
  % IEEE copyright notice, bottom-left of first page only.
  \fancyfoot[L]{\footnotesize 979-8-3195-3824-6/26/\$31.00~\copyright2026~IEEE}%
}
...
% CAMERA-READY: real author block (must match the CMT author record)
\author{
\IEEEauthorblockN{Sangmin Kim}
\IEEEauthorblockA{\textit{Dept. of Electrical and Computer Engineering} \\
\textit{Sungkyunkwan University}, Suwon, Republic of Korea \\
also with CASELAB Co., Ltd., Anyang, Republic of Korea \\
kimsang.m@g.skku.edu}
\and
\IEEEauthorblockN{Byeongjun Kim}
\IEEEauthorblockA{\textit{Dept. of Intelligent Robotics} \\
\textit{Sungkyunkwan University}, Suwon, Republic of Korea \\
also with CASELAB Co., Ltd., Anyang, Republic of Korea \\
byeongjunkim@g.skku.edu}
\and
\IEEEauthorblockN{Tae-Yong Kuc}
\IEEEauthorblockA{\textit{Dept. of Electrical and Computer Engineering} \\
\textit{Sungkyunkwan University}, Suwon, Republic of Korea \\
Corresponding author: tykuc@skku.edu}
}

\maketitle
\thispagestyle{raaiconfirst}
...
scan-to-simulation pipeline that converts a registered terrestrial laser
...
Table~\ref{tabcmp} contrasts the main routes from a captured indoor
space to a robot-usable simulation environment. A direct head-to-head
benchmark is not yet practical---published systems target different
simulators and input modalities and rarely release end-to-end
code---so the table compares the characteristics reported by each
line of work against those measured for our pipeline in
Section~\ref{sec:results}: our conversion is fully automatic,
completes in 78.5 s, and is validated metrically both at the scene
level and end to end.

\begin{table}[htbp]
\caption{Routes from a captured indoor space to a simulation
environment (characteristics as reported by each line of work)}
\begin{center}
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Approach} & \textbf{Autom.} & \textbf{Turnaround} &
\textbf{Metric valid.} \\
\hline
Manual modeling \cite{tao2019} & manual & days/room & rarely rep. \\
Scan-to-BIM \cite{scan2bim} & semi-autom. & min.--hours & element-level \\
Mesh real-to-sim \cite{gibson} & offline pipeline & not reported & visual focus \\
\textbf{This work} & \textbf{automatic} & \textbf{78.5 s} &
\textbf{$\leq$2\%; 5.5 cm} \\
\hline
\end{tabular}
\label{tabcmp}
...
Fig.~\ref{figpipe} shows the system, which consists of an offline
conversion stage (top row) and an online twin runtime (bottom row).
Offline, the merged BLK360 E57 export first passes through
preprocessing (Section~\ref{sec:preproc}): voxel thinning followed by
RANSAC floor-plane estimation, which fixes the gravity-aligned
reference frame. From this common frame the pipeline derives three
artifacts: (i) a 2D occupancy grid, obtained by rasterizing a
robot-relevant height band and exported both as ROS map files and,
after greedy box merging, as the invisible collision layer
(Section~\ref{sec:occgrid}); and (ii) a dense colored point cloud,
voxel-downsampled and embedded as the visual layer
(Section~\ref{sec:vislayer}). The collision and visual layers are
composed into a single Isaac Sim USD stage together with the AMMR
robot model (Section~\ref{sec:robot}). Online, the real robot's
localized pose is streamed at 30 Hz through a ROS 2 DDS bridge and
applied to the simulated robot, while navigation goals picked in the
simulator viewport travel the reverse path to the robot's on-board
Nav2 stack; a one-shot anchor calibration aligns the two coordinate
frames (Section~\ref{sec:bridge}).
...
\label{sec:preproc}
The test space is an industrial robot test room of approximately
$13\times9$ m. It was selected on three grounds. First, it is the
actual deployment environment of the AMMR and contains genuine
industrial furnishing---workbench rows, racks, equipment cabinets,
and monitor clusters---so the reconstruction is exercised on the
clutter distribution the twin must ultimately serve, rather than on a
staged mock-up. Second, the room combines surface types that are
known to stress laser-scan-based reconstruction (glass windows,
dark light-absorbing equipment, and reflective flooring), making it a
conservative test case for the conversion. Third, the facility
allows controlled, repeatable access, which is required for the
tape-measured ground-truth campaign and the on-robot experiments in
Section~\ref{sec:results}; conveniently, the robot itself was present
during scanning, which later provides the calibration anchor of
Section~\ref{sec:bridge}. The room was captured with a Leica BLK360
from multiple viewpoints; scan placement followed a
visibility-coverage plan, and the registered stations were exported
as a single E57 file (68.5 M points, 1.3 GB). The pipeline first estimates the dominant floor plane with
RANSAC on a voxel-thinned subset (implemented with
Open3D~\cite{open3d}), yielding the gravity-aligned
...
of 0.013 m, chosen at approximately 0.85 times the voxel size so points
...
(TF $map\rightarrow base$, AMCL~\cite{dellaert1999}) as a 30 Hz odometry stream; the
...
\label{eq:anchor}
\end{equation}
and is reused for all subsequent poses and goals. The accuracy and
failure modes of this single-anchor scheme are analyzed in
Section~\ref{sec:discussion}.

\section{Experimental Results}
\label{sec:results}
All experiments ran on a desktop workstation (AMD Ryzen 9 9900X,
24 threads, 32 GB RAM, NVIDIA RTX PRO 4000 Blackwell 24 GB, Isaac
Sim 5.1, ROS 2 Jazzy).
...
Resource utilization is modest. The conversion runs entirely on the
CPU: the floor-estimation and rasterization stage peaks at 8.1 GB of
resident memory---dominated by holding the full 68.5 M-point cloud in
RAM---at an average CPU utilization of 2.4 cores (measured with
\texttt{/usr/bin/time}); no GPU is required until simulation. The
simulator process itself occupies 3.1 GB of GPU memory with the full
scene and robot articulation loaded in headless physics-only mode,
leaving ample headroom on the 24 GB device for rendering and
additional robots.
...
factor (RTF) of 5.6; with rendering enabled the simulator sustains
...
3.4 m sweep of the room; the mirrored pose advanced at the AMCL update
rate of the onboard localization. Across two sessions on consecutive
days, all four navigation goals issued by dragging the target marker
in the simulator viewport drove the robot to the corresponding
physical locations (Fig.~\ref{figgoal}); Table~\ref{tabgoal} lists
the individual trials. The twin-frame arrival error of
0.19--0.23 m (mean 0.21 m) is consistent with the Nav2 goal
tolerance, at commanded speeds up to 0.52 m/s.

\begin{table}[htbp]
\caption{Simulator-issued navigation-goal trials}
\begin{center}
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{|c|c|c|l|}
\hline
\textbf{Trial} & \textbf{Session} & \textbf{Error [m]} &
\textbf{Note} \\
\hline
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_pair_room.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_pair_goal.png}}
\caption{Simulator-issued navigation goal. Left: the physical robot at
the reached location; right: the twin after arrival, parked on the goal
marker.}
\label{figgoal}
\end{figure}
...
(mean absolute error 6.4 cm, or 1.3\%), i.e., the scan-to-simulation
conversion itself preserves metric scale to centimeter level.
...
absolute error of 5.5 cm (RMSE 7.4 cm, max 12.5 cm, signed bias
$+$2.8 cm; Fig.~\ref{figeval}). The largest deviation occurred on the segment
...
\subsection{Discussion: scalability and limitations}
\label{sec:discussion}
\subsubsection{Scalability} Every pipeline stage is linear in the
number of input points, and the measured bottlenecks are I/O-bound
(Table~\ref{tabtime}), so conversion time scales roughly linearly
with scan size; the occupancy grid already covers the full
$32.7\times23.7$ m scan extent at 0.05 m resolution. Larger
facilities enter through two paths: more TLS stations, whose
registration is handled at capture time before the E57 export, and a
proportionally larger visual layer, which can be bounded by raising
the voxel size (the 485 k-point variant converts in 36.5 s) or by
tiling the stage per room and loading tiles on demand. The main
memory constraint is holding the merged cloud in RAM (8.1 GB for
68.5 M points), which for facility-scale scans beyond roughly
$10^8$ points would call for chunked streaming of the E57 input.
For dynamically changing environments the pipeline itself offers a
pragmatic mitigation---a facility change is absorbed by rescanning
and rerunning the 80-second conversion---but continuous object-level
scene updating from on-robot sensors remains future work.

\subsubsection{Limitations of anchor-based calibration} The one-shot
calibration of \eqref{eq:anchor} inherits the localization error of
the single pose sample it is computed from: any AMCL error at anchor
time becomes a permanent bias of the session. In our two on-site
sessions the recovered offsets differed by 0.60 m and 9.1$^\circ$,
reflecting run-to-run differences in AMCL initialization rather than
scene error; our operating procedure therefore verifies the overlay
visually after anchoring and re-anchors if the twin is misaligned.
The scheme also presupposes a physically identifiable anchor pose in
the scan (here, the robot's own scanned footprint). Averaging over
multiple anchor poses, or refining the offset by matching the live
2D-lidar scan against the TLS cloud, would remove the
single-sample dependence.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_eval.png}}
...
the physical robot. Future work includes automatic semantic
decomposition of the scene into articulated objects, mesh
reconstruction for contact-rich manipulation, continuous object-level
scene updating from on-robot sensors for dynamically changing
facilities, and multi-robot twins.

\section*{Acknowledgment}
The authors thank the members of the Control and Robotics Laboratory
at Sungkyunkwan University for helpful discussions and for supporting
the on-site experiments.
...
[Online]. Available: https://leica-geosystems.com/products/laser-scanners (accessed Jul. 2026)
\bibitem{isaacsim} NVIDIA, ``Isaac Sim,'' [Online]. Available:
https://developer.nvidia.com/isaac/sim (accessed Jul. 2026)
...
pp. 162--171, 2015.
\bibitem{gibson} F. Xia, A. R. Zamir, Z.-Y. He, A. Sax, J. Malik, and
S. Savarese, ``Gibson Env: Real-world perception for embodied
agents,'' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit.
...
pp. 737--744.
\bibitem{open3d}
Q.-Y. Zhou, J. Park, and V. Koltun, ``Open3D: A modern library for 3D data processing,'' \textit{arXiv preprint arXiv:1801.09847}, 2018.

\bibitem{dellaert1999}
F. Dellaert, D. Fox, W. Burgard, and S. Thrun, ``Monte Carlo localization for mobile robots,'' in \textit{Proc. IEEE Int. Conf. Robot. Autom. (ICRA)}, 1999, pp. 1322--1328.

\bibitem{cyclonedds} Eclipse Foundation, ``Eclipse Cyclone DDS,''
[Online]. Available: https://cyclonedds.io (accessed Jul. 2026)
\end{thebibliography}
