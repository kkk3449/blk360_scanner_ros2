...
laser scan and emits them as a VDA~5050-style JSON message. A deterministic stage segments the colored point cloud and fills the
explicit model (pose, dimensions, color) with a provisional class from an
open-vocabulary 3D classifier; a vision--language model (VLM) stage then
verifies each class from multi-view renders under forced tool-use and
infers the implicit affordance model. We show that the obvious multi-view strategy ---
...
human audit. Full-scene verification costs \$2.6; prompts, settings, and audits are
released.
\end{abstract}
...
occupancy grid: the robot must know \emph{what} occupies the space,
\emph{where}, and \emph{how} it can be interacted with. The
...
displaced). TOSM-style databases drive navigation frameworks and robot fleets, and
their content maps naturally onto the JSON conventions of
VDA~5050~\cite{vda5050}, the emerging AGV-to-master-control interface
standard.

In practice, the semantic-object database for a new facility is authored
by hand. Terrestrial laser scanners (TLS) such as the Leica BLK360 capture
a dense, colored as-built model in minutes, and scan-to-BIM
research~\cite{patraucean2015} recovers structural geometry, but
object-level semantics remain the gap: open-vocabulary 3D classifiers
...
discards. We render each object from four azimuths and study how a VLM
should consume them. The naive strategy --- all views in one prompt
(\emph{early fusion}) --- \emph{hurts}: sparse oblique point renders
invite over-confident misreadings that contaminate the joint judgment.
Classifying each view independently and voting by confidence
(\emph{late fusion}), the analogue of MVCNN-style
aggregation~\cite{su2015mvcnn}, recovers most of the lost reliability and
exposes inter-view disagreement as an uncertainty signal. The contributions are:
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\columnwidth]{fig_pipeline.png}
...
\textbf{Scan-to-model.} As-built modeling from TLS targets structural
elements~\cite{patraucean2015}; object semantics are left to learned 3D
segmentation~\cite{qi2017} or open-vocabulary encoders such as
...
route to 3D shape recognition (MVCNN~\cite{su2015mvcnn}); we revisit
early-vs-late fusion for VLM prompts over \emph{sparse point} renders.
\textbf{VLMs in robotics.} Recent systems use VLMs for open-set scene
graphs~\cite{gu2024conceptgraphs} and language-driven
maps~\cite{huang2023vlmaps}; we use one in a narrower, auditable role:
verifying one object at a time against measured geometry.
...
downsampled at 3\,cm. Structural planes are removed by seeded RANSAC
(300 triples per plane, 5\,cm inlier threshold, SVD refit; only
near-horizontal $|n_z|\ge0.85$ or near-vertical $|n_z|\le0.2$ planes are
removed; stop after ten planes, below 2{,}000 remaining points, or when
the best plane holds $<$4\% of them or is slanted). Objects are then
segmented by DBSCAN~\cite{ester1996} ($\varepsilon=0.30$\,m,
$\mathrm{minPts}=100$), which yields 30 clusters on our scene; every
cluster whose footprint exceeds 2.5\,m is re-clustered at
$\varepsilon=0.20$\,m (split accepted only if it yields $\ge$2
sub-clusters of $\ge$100 points), raising the count to the 57 objects
evaluated in Sec.~\ref{sec:setup} with no further filtering.
Segmentation stays geometric because ScanNet-trained segmenters did not
transfer to this scene (SPFormer labeled only 51\% of points, PTv3 called
46\% \emph{wall} after wall removal). For each object Stage~A
...
512$\times$512, point size~8, red 3D box) are exported; smaller point
sizes produced speckle-like views that misled both the VLM and the human
auditors (Sec.~\ref{sec:study}). Stage-A fields are measured, not
generated; Stage~B never modifies them.
...
vocabulary --- not generated per object but one operator-supplied
scene-level list (\emph{chair, TV, monitor, Mobile Robot, desk} here)
passed unchanged to every call. The decision rule is three-way: keep a
correct label; if wrong, prefer a fitting candidate; else emit an
open-vocabulary label. Guardrails address point-render failure modes
(Fig.~\ref{fig:prompt}): dimensions as a hard constraint, oversize
clusters as merged objects, thin-panel disambiguation, no parts not
outlined by points, and a keep-or-\emph{clutter} fallback.
...
signal. Fig.~\ref{fig:votes} shows a unanimous case (fused confidence
1.0). Conversely, a single contaminated view is simply outvoted (a
tall-backed chair whose frontal view read \emph{Mobile Robot} fused to
\emph{chair} 3:1), whereas inside one early-fusion prompt the same view
can drag the joint judgment toward the wrong label.

\begin{figure}[t]
\centering
\includegraphics[width=0.5\columnwidth]{fig_votes.png}
...
replace --- prefer a fitting candidate, otherwise the most accurate
common-noun label; never `unknown'. \emph{Size sanity:} measured
dimensions are a HARD constraint; an oversize cluster is merged objects
--- label the dominant structure or `clutter'. \emph{Sparse-render
caution:} never claim parts not outlined by points. \emph{Panel rule:}
thin flat objects are keyboard / display / shelf --- never ladder.
\emph{Chair gate:} visible seat pan 0.3--0.6\,m plus backrest. If
uncertain, keep a size-compatible label. Answer ONLY via
\texttt{report\_symbolic}.\\[1pt]
...
message with a VDA~5050-style header (Table~\ref{tab:mapping}); TOSM
fields with no standard slot travel in the extensible \texttt{properties}
map, so consumers that only know the base schema can still read it.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_cards.png}
...
colored points, 57 segmented objects) with Claude Sonnet 4.6. The six
configurations (Table~\ref{tab:configs}) form a $2\times2$ design over
view count (1 vs.\ 4, early fusion) and render/prompt generation (sparse
renders + base prompt vs.\ dense renders + guardrail prompt), plus a sixth
that differs from four-view early fusion (dense) \emph{only} in the
fusion rule: render density and guardrails were upgraded together and are
not separable here, whereas the fusion contrast is isolated. Metered
cost: late fusion \$2.63 per scene (285 calls, \$0.046/object); early
fusion \$1.27; single view \$0.87.

Labels are graded by two independent references. (i)~\emph{Render
audit:} two of the authors audited all 57 objects against the dense
...
(ii)~\emph{On-site inventory:} the facility operator then assigned every
object a ground-truth type by physical inspection, without reference to
the render audit (43 confirmed, 12 plausible, 2 unknown; 7 clusters are
ceiling-soffit structure remnants). Inventory scoring is strict --- a
label counts only if it equals the inventory type, so a conservative
\emph{clutter} on a real chair is wrong --- on all 57 objects and on the
36 confirmed non-structure objects. Audit-vs-inventory agreement is
79--95\% over the five VLM configurations (Cohen's $\kappa$ 0.53--0.89;
89\%, $\kappa=0.79$ for late fusion): the audit is a faithful but
imperfect proxy whose optimistic calls are confined to \emph{handrail}
labels on soffit remnants and whose single pessimistic call concerns an
object the inventory itself marks unknown.
...
lenient; 34 vs.\ 30 inventory matches): a contaminated view is outvoted.
It does not overtake the best single canonical view (60\%/81\% audit;
44/57 inventory matches, 77\%, 95\% Wilson CI 65--86\%, vs.\ 60\%, CI
47--71\% for late fusion): on sparse point clouds extra views add noise
faster than evidence, and the vote spends much of its gain on caution.
Its larger \emph{plausible} bucket --- split votes often land on
\emph{clutter} --- counts as wrong under the strict inventory, which is
why late fusion exceeds the classifier by only one object there (34 vs.\
33). We therefore claim late fusion as the safer multi-view rule, not as
an accuracy gain over a well-chosen single view; the measurable value of
the multi-view stage is the reliability signal below.
...
\emph{Repeatability.} Because the VLM samples stochastically, we later
re-ran the late-fusion protocol twice on the 50 non-structure objects.
Labels agreed between the re-runs on 82\% of objects; accuracy on the 36
confirmed objects was 28, 27, and 27 across the three runs ($\pm$1),
unanimity precision 16/17, 20/21, and 19/20 (94--95\%), split-vote
accuracy 45\%, 28\%, and 30\%. Rankings are stable to sampling, but the
single-scene 57-object sample yields intervals of roughly $\pm$12 points,
so all comparative claims are scoped to this scene.
...
label wrong. A label that fools both the VLM and a human at the same
renders was still caught by vote splits (these objects never reached
unanimity), the strongest argument for the gating above; the incident
also motivated a structure-remnant filter now applied in Stage~A.
...
confidence, one-sentence reason). Mean fused confidence was 0.71; the 17
unanimous objects carry 1.0 by construction, matching the observed 94\%
unanimity precision. By design the records are a reviewable draft, not
ground truth: the model stays user-editable and every amendment is logged.
An owner review exercised this loop on the implicit model, correcting two
movability entries (a wheeled machine judged fixed, a wall-mounted display
judged movable) at negligible cost.
...
votes, forced-tool JSON, stored reasons). All 741 tool calls across the
five VLM configurations returned schema-valid JSON. Remaining errors
concentrate where the render itself is ambiguous (texture-poor slabs,
merged clusters, sparse strips), pointing to upstream fixes --- finer
segmentation, mesh renders --- not to the fusion protocol. Absolute strict
accuracy (46--60\% audit, 53--77\% inventory) reflects a deliberately
hard test bed: a cluttered showroom scanned once, without cleanup. Limits:
a single scene and 57 objects; confounded render-density and prompt
factors; an inventory compiled by one operator. Multi-scene evaluation
with per-factor ablation is future work.
...
on-site inventory, showed that the intuitive all-views-at-once prompt
degrades verification on sparse point renders, whereas per-view
classification with confidence-weighted late fusion is the strongest
multi-view strategy (wrong labels 24$\rightarrow$15, 74\% lenient),
although a single canonical view remains the most accurate configuration
on this scene. The robust benefit of the multi-view stage is its vote
signal: unanimity gives a 94--95\%-precise gate across three independent
runs at \$2.6 per scene. Future work includes mesh renders, finer
segmentation, multi-scene evaluation, and coupling the records to place
