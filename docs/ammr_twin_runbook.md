...
- **계획 3번 호스트 분리 (9/22 완료, `docs/split_deployment.md`):** 노트북 콘솔(도커 이미지 `docker/console/`, `~/Downloads/ammr-console_20260922.tar.gz` 620 MB) ↔ Isaac/데이터 PC(브로커 :1883, `kg_data_server.py` :8090, Isaac 트윈) ↔ 로봇(WiFi). 기기 간은 MQTT+HTTP만, DDS 없음. KG 권위본은 PC; 콘솔이 pull(10 s 폴링)·owner 편집 push. `bringup_vda5050.sh console|dt|agv` + `BROKER/KG_SOURCE_URL/SCENE_URL`. 검증: 랜 IP 경유 3역할 분리 기동, 컨테이너 콘솔에서 미션·KG 편집 왕복, MQTT 랜 20 ms.
