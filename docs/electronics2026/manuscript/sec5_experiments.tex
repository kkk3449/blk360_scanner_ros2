...
case in Sec.~\ref{sec:exp-ablation}.
...
\paragraph{Escalation on/off.} The base-4 column of
Table~\ref{tab:verification}: +11.1/+6.3 points.
...
everything else, so it ships as a targeted defense only.

\paragraph{Uni3D vocabulary.}
Generic-30 66.7\% $>$ curated-20 48.7\% $>$ owner-18 33.3\%
(run-to-run variance $\sim$5 points), confirming the embedding-level domain
gap that motivates verification.
...
\paragraph{Cross-floor stress scene and render-source ablation.}
The cafeteria scene (Sec.~\ref{sec:setup}) probes the same protocol,
same verifier, on a furnishing domain the campaign never saw: carpeted
floor, fabric sofa booths, planter partitions. The backbone transfers
mechanically (57M points $\rightarrow$ 28 clusters in 13.5\,s after an
automatic yaw alignment), but verification does not: escalated accuracy
reaches only 35\% against confirmed owner GT (46.7\% when
fragment clusters are excluded from grading), and unanimous precision
falls to 57.1\%---two unanimous keyboard phantoms on carpet-noise
clusters, a chair and a table pedestal unanimously absorbed as
\texttt{clutter}. The errors are structured, not diffuse: the scene's
dominant object family (sofas) enters the pipeline pre-broken---split
into six cushion fragments and two sofa-table merges---and every
fragment is absorbed; carpet pile leaves seven residual floor-noise
clusters above the RANSAC floor plane (a floor-material analogue of the
ceiling remnants of Sec.~\ref{sec:exp-seg}); and the confidence gate
quarantines 9 of 16 wrong labels as unverified while 4 unanimous errors
pass. Re-rendering the identical 512-px view sets from full-resolution
re-associated points (rather than the 3\,cm working cloud) isolates the
render-quality term at unchanged protocol and cost ($\sim$\$2.5/scene):
every whole-object error repairs---the keyboard phantoms vanish, both
chairs recover (including the unanimous one), and both doors resolve
correctly, one via escalation---lifting escalated accuracy to 40\%
(53.3\% excluding fragments) and unanimous precision to 62.5\%; but no
sofa fragment is recovered, and two planters regress. A final cell
completes the model $\times$ render-source square: Sonnet~5 on the same
full-resolution sets reaches 60\% (80\% excluding fragments, unanimous
precision 80\%)---the decisive model of the upgrade check converts its
decisiveness into accuracy once renders are informative, consistent
with its 1024-px behavior in Table~\ref{tab:vlmcmp}---yet it recovers
\emph{zero} of the seven sofa clusters and both planters stay
mislabeled. The levers separate cleanly: segmentation bounds what any
verifier can see, render quality bounds what a given verifier resolves,
and model generation buys accuracy only on top of both---so
fragmentation-induced absorption is reachable by none of them.
Together with the
upgrade check, the stress scene fixes the reading of
Table~\ref{tab:verification}: tier precision is a scene- and
domain-scoped measurement to be re-estimated per deployment---cheaply,
via a small owner-confirmed probe set---not a property of the
architecture.

\paragraph{Segmentation parameters.} See Sec.~\ref{sec:exp-seg}.
