...
re-derives per-epoch names from the same completed partition
(Sec.~\ref{sec:place}): exhibition-era \texttt{control\_panel\_station} and
\texttt{machine\_operation\_area} become storage- and clutter-dominated at
T2 and \texttt{machine\_operator\_station}/\allowbreak
\texttt{electrical\_cabinet\_area} at T3, at $\sim$\$0.03 per epoch.
