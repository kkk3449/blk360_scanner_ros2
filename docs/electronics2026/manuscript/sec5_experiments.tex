...
within a generation the smaller tier splits more votes, resolves fewer,
and ends up \emph{more} expensive end-to-end (\$3.40 vs.\ \$3.00)
for lower type accuracy: a small model's per-call price advantage is
consumed by its own indecision. Second, all three current-generation
...
unanimous, 6 escalations), and the premium tier's extra deliberation
...
the deployment's own data --- and a purchasing guide: pick the
cheapest verifier whose escalation rate has saturated.
...
\textbf{247} & $\sim$\$2.5 \\
GPT-5.6 (luna) & 21 & 7 & 7 & \textbf{7/13} & 287 & $\sim$\$1 \\
Claude Opus 5 & 17 & 8 & 10 & \textbf{7/13} & 319 & $\sim$\$16 \\
Claude Sonnet 4.6 & 15 & 6 & 14 & 6/13 & 335 & \$3.00 \\
Claude Haiku 4.5 & 9 & 9 & 17 & 5/13 & 383 & \$3.40 \\
\bottomrule
\end{tabular}

{\footnotesize $^{1}$ canonical-synonym type match on the clusters that
geometrically match a ground-truth object. $^{2}$ 4.x-generation Claude rows at
vendor list pricing; 2026-generation figures ($\sim$) are estimated
from measured tokens at family list rates (Sonnet~5: 0.63M/40K;
Opus~5: 0.79M/53K; GPT-5.6: 0.42M/19K in/out).}
