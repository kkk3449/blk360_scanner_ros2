...
\label{fig:baselines}
\end{figure} The T3 epoch supplied a stress case: at 99.6\% coverage, inter-object
...
conditional refiner of Sec.~\ref{sec:exp-seg}.
...
PTv3 (semantic) & --- & --- & 17/20 fire & 46\% ``wall'' after removal \\
...
94K-point residual); out-of-room bleed-through accounts for another
...
panel band --- yields \emph{exactly} the owner's inventory: ten
monitors (five columns of stacked pairs), the desk beneath them, and,
from the off-wall remainder, a wheelchair robot and a single table leg.
The gate turns ``too diffuse to be an object'' from a discard decision
into a decomposition trigger.
...
real objects ($\sim$8\%). The fusion also quantifies label fragility:
...
Figure~\ref{fig:wallcompound} shows the wall-compound machinery end to
end on the room's monitor wall.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig17_wall_compound.png}
\caption{Automatic wall-compound decomposition (no owner input).
(a)~As detected: a single 6\,m cluster mixing wall paint, ten dark
monitor screens, support posts, and the desk --- confidently verified as
one ``electrical cabinet'' by the baseline pipeline. (b)~After the
wall-compound gate: the mapped wall band is stripped, the support plane
separates the desk, u-density valleys cut the panel bank into columns,
a conditional z-cut splits stacked rows, and a square-up pass re-assigns
boundary points --- yielding nine of the ten owner-confirmed monitor
units plus posts, desk, and under-desk storage.}
\label{fig:wallcompound}
\end{figure}
...
\midrule
Unanimous precision & \multicolumn{2}{c}{100\% (21/21)} & \multicolumn{2}{c}{94.4\% (of 25)} \\
...
interface must demote. A separate check evaluates the room-scoped T1 objects
...
structure phantoms --- including the staircase band and a fluorescent
fixture verified as a keyboard at 3.3\,m --- are eliminated upstream.
...
is \texttt{inserted} under a new ID; only the insertion is re-verified
(\$0.23). (D)~Partial occlusion: both cropped objects still match at 20\%,
...
verifier tiers and vendors makes escalation an economic instrument ---
spend tracks verifier uncertainty. Within one vendor, the smaller tier
splits its base votes more often (26 vs.\ 20 of 35 objects escalate),
resolves fewer of them, and ends up \emph{more} expensive end-to-end
(\$3.40 vs.\ \$3.00) for slightly lower type accuracy: the per-call
price advantage of a small model is consumed by its own indecision.
A cross-vendor frontier model (GPT-5.6, non-reasoning mode for parity)
extends the same monotone pattern --- the most decisive verifier
(21 unanimous, only 14 escalations) issues 287 calls where the small
tier needs 383, and posts the best type accuracy on the recalled set.
Escalation volume is thus a label-free, online proxy for verifier
quality on the deployment's own data.
...
\midrule
GPT-5.6 (luna) & \textbf{21} & 7 & \textbf{7} & \textbf{7/13} &
\textbf{287} & $\sim$\$1 \\
Claude Sonnet & 15 & 6 & 14 & 6/13 & 335 & \$3.00 \\
Claude Haiku & 9 & 9 & 17 & 5/13 & 383 & \$3.40 \\
\bottomrule
\end{tabular}

{\footnotesize $^{1}$ canonical-synonym type match on the clusters that
geometrically match a ground-truth object. $^{2}$ Claude rows at vendor list
pricing; the GPT-5.6 figure is estimated from measured tokens
(0.42M in / 19K out) at GPT-5-family rates.}
\end{table} \emph{Segmentation parameters}: see
Sec.~\ref{sec:exp-seg}.
