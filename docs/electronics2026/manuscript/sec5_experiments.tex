...
owner-minute economy that Sec.~\ref{sec:exp-owner} argues the model
should optimize.
...
render source (identical 512-px, 8-azimuth protocol throughout).
Accuracy is escalated accuracy on confirmed GT ($n{=}20$; the
fragment-excluded column drops the six sofa-fragment and pedestal
clusters, $n{=}15$); unanimous precision is over confirmed unanimous
votes. In every cell, 0 of the 7 sofa clusters are recovered.}
\label{tab:cafe}
\small
...
accuracy (confirmed GT) and unanimous precision across verifier
$\times$ render source.}
\label{fig:cafe}
\end{figure}
...
and a chair and a table pedestal unanimously absorbed as
...
split into six cushion fragments and two sofa--table merges, and every
...
16 wrong labels as unverified while the 4 unanimous errors pass.
...
domain-dependent. The render-source lever carries the same caveat: a
symmetric control that re-renders the \emph{showroom} from
full-resolution points reverses the effect, dropping escalated
accuracy from 88.9\% to 80.6\% (base 77.8\%$\rightarrow$69.4\%;
unanimous precision holds at 100\%, trigger 58\%$\rightarrow$66\%).
The added texture invites over-specific labels on industrial
equipment---a junction box read as a pipe assembly, the wall TV as a
ladder---where the coarser working-cloud render had left a correct
