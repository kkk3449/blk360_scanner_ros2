...
(Table~\ref{tab:vlmcmp}): running the identical Stage-B protocol with a
smaller VLM tier inverts the expected cost story --- the weaker verifier
splits its base votes more often (26 vs.\ 20 of 35 objects escalate),
resolves fewer of them (9 vs.\ 15 unanimous verifications), and ends up
\emph{more} expensive end-to-end (\$3.40 vs.\ \$3.00) for slightly lower
type accuracy against the owner inventory. Because escalation spends in
proportion to verifier uncertainty, model quality surfaces as total cost
rather than only as accuracy; the framework's per-call price advantage
of a small model is consumed by its own indecision.
...
\toprule
Verifier & Verified & Escalated-resolved & Unverified &
Type-correct$^{1}$ & Cost \\
\midrule
Claude Sonnet & \textbf{15} & 6 & \textbf{14} & \textbf{6/13} &
\textbf{\$3.00} \\
Claude Haiku & 9 & 9 & 17 & 5/13 & \$3.40 \\
\bottomrule
\end{tabular}

{\footnotesize $^{1}$ canonical-synonym type match on the clusters that
geometrically match a ground-truth object.}
