...
trigger rate (52--58\% of objects), and per-call cost is $\sim$\$0.009; a
40--80 object scene verifies for \$3.5--5. The incremental design changes
the cost model qualitatively: a repeat scan costs \$0 (hash carry-over) and
a changed scene costs in proportion to its change (2.7\% of rebuild in our
...
(Sec.~5.3's unanimous-but-wrong cases; the escalated keyboard trap that
survives gating in both scenes). Verifier-bounded gating is a property users
must design around, and we report it as such.
...
limitation. The place layer depends on manually authored polygons from a
prior deployment; automatic room segmentation is deliberately out of scope.
