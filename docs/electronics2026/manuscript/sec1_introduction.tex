...
Sec.~5.6), indicating an embedding-level domain gap rather than a class-list
...
claims in our LLM-only benchmark condition (Sec.~5.4). For a robot, a
...
geometry changed within two weeks (Sec.~5.5).

This paper presents a framework that addresses all three failures with one
design principle: \emph{separate what can be made deterministic from what
...
owner-confirmed ground truth, through five benchmark familes: segmentation
...
+11.1/+6.3 points, gated querying +31 points over LLM-only with traps
blocked, re-verification at 2.7\% of rebuild cost---the study surfaces
