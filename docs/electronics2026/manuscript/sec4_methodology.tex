...
timestamps. Edges (spatial relations recomputed from explicit models) are
scoped to the map and revision. The result is exported both as a canonical
...
verified object layer. Manually authored section polygons (from our 2025
deployment of the same room) are registered into the scan frame by 2D
occupancy-map alignment and completed to a \emph{full partition} of the room
mask: every free cell is assigned to its containing polygon, or to the
nearest one where the manual polygons leave gaps---a Voronoi-style
completion in the spirit of occupancy-grid room
segmentation~\cite{bormann2016room,luperto2022robust}. Each object's
\texttt{isInsideOf} therefore resolves to exactly one place. Each place is
then summarized by its
...
its ring code. Across our three scan epochs the same polygons re-derive
names that track the room's actual use (the docking section:
exhibition-era \texttt{doorway\_passage} $\rightarrow$ storage-era
...
Fig.~\ref{fig:placemap} shows the resulting T3 place layer. Fully automatic
room segmentation (as in DK-SMF's SLIC approach) and the robot layer of
TOSM are out of scope here.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{fig11_place_map.png}
\caption{Object-grounded place layer at T3. Registered section polygons are
completed to a full partition of the room mask (Voronoi-style
assignment~\cite{bormann2016room,luperto2022robust}) and carry LLM-derived
names computed from verified-object ring codes; unverified objects (open
markers) do not contribute, so the confidence gate propagates into place
semantics.}
\label{fig:placemap}
\end{figure}
