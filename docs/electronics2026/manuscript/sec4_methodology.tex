...
objects (named stars), shown after the owner-feedback revision
(Sec.~\ref{sec:exp-verify}): refuted phantoms (a verified keyboard,
staircase, and book that do not exist in the room) have been removed
from the graph, the poster stand carries its owner-corrected label, and
the recovered TV is the key object of its region.}
\label{fig:relmap}
\end{figure}
...
after the owner-feedback revision (Sec.~\ref{sec:exp-verify}): with the
phantoms refuted, the region once named after a phantom staircase renames
itself \texttt{display\_briefing\_area} around the recovered TV and
poster stand.}
\label{fig:placemap}
\end{figure}
