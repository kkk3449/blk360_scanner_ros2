...
recipe: the map is partitioned by SLIC superpixel
segmentation~\cite{achanta2012slic} with the number of superpixels as the
governing parameter (7 requested here), only regions inside free space are
retained, fragments below an area threshold (1.5\,m$^2$) are discarded,
and each region's centroid is the moment-based center of its cells (cf.
grid-map room segmentation~\cite{bormann2016room,luperto2022robust}). Each
...
regions~\cite{achanta2012slic} over the free space of the nav-stack
occupancy grid (white voids are obstacles) carry LLM-derived names computed
from verified-object ring codes; unverified objects (open markers) do not
