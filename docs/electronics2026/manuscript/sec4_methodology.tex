...
and revision; Fig.~\ref{fig:relmap} shows the resulting relation-annotated
map at T3. The result is exported both as a canonical
...
\caption{Relation-annotated TOSM map (T3): \texttt{isNextTo},
\texttt{isOn}, and \texttt{isAboveOf} edges recomputed from the explicit
models at every revision, drawn over the place layer of
Fig.~\ref{fig:placemap} (pastel)---object--place containment
(\texttt{isInsideOf}) is the region an object lies in---with verified
(filled) and unverified (open) object nodes.}
\label{fig:relmap}
\end{figure}
