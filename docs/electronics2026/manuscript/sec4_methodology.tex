...
signal (100\% precision on both scenes' confirmed ground truth, Sec.~5.3),
...
across-epoch scans pay only for changed clusters (2.7\% of full cost in our
measurement, Sec.~5.5).
...
stated in the schema, not in behavioral instructions. With the gate in the
data rather than the prompt, the model cannot assert an unverified label
without visibly contradicting its input: the four unverified-label traps are
blocked in the gating-style ablation (4/4, Sec.~5.6), while in the
end-to-end benchmark one count-phrased trap still leaks through the
quarantine field itself (3/4, Sec.~5.4). Traps whose label was wrongly
\emph{verified} lie beyond any status-based gate---a bound we quantify in
Sec.~5.4.
...
occupancy-map alignment; each object's \texttt{isInsideOf} is computed by
point-in-polygon on its anchor. Each place is then summarized by its
...
names that track the room's actual use (exhibition-era
\texttt{control\_panel\_station} $\rightarrow$ storage-era
\texttt{storage\_area} $\rightarrow$ test-era \texttt{operator\_workstation};
Sec.~5.5), demonstrating that place semantics stay current without manual
re-annotation. Automatic room segmentation (as in DK-SMF's SLIC approach) and
the robot layer of TOSM are out of scope here.
