...
re-annotation. Fig.~\ref{fig:placemap} shows the resulting T3 place layer.
Automatic room segmentation (as in DK-SMF's SLIC approach) and
the robot layer of TOSM are out of scope here.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{fig11_place_map.png}
\caption{Object-grounded place layer at T3: registered section polygons
carry LLM-derived names computed from verified-object ring codes; unverified
objects (open markers) do not contribute, so the confidence gate propagates
into place semantics.}
\label{fig:placemap}
\end{figure}
