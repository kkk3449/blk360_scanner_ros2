...
%DIF DEL sensors-4464625-submitted.tex   Sun Aug  9 00:33:22 2026
%DIF ADD sensors-4464625-r1.tex          Sun Aug 16 09:13:27 2026
...
%DIF 78c78
%DIF < \Title{Occlusion-Aware Visibility Coverage for Stop-and-Scan 3D Mapping with a Terrestrial Laser Scanner}
%DIF -------
\Title{Occlusion-Aware Visibility Coverage for Robotic Stop-and-Scan 3D LiDAR Mapping} %DIF > 
%DIF -------
...
%DIF 112-114c112-116
%DIF < % NOTE: restore \conference{...} once the ICCAS 2026 paper is accepted; the mdpi.cls
%DIF < % footnote prefix reads "published in", which is inaccurate while it is under review.
%DIF < %\conference{the 2026 International Conference on Control, Automation and Systems (ICCAS), which introduced an isotropic-disk stop-and-scan trigger. The present work replaces the disk model with an occlusion-aware ray-cast visibility model, adds a controlled ablation and a parameter sensitivity analysis, and reports a fully autonomous on-hardware disk-versus-visibility comparison on a mobile platform with per-run survey-grade offline registration of the acquired BLK360 scans.}
%DIF -------
% NOTE: \conference{} hard-codes the prefix "This paper is an extended version of our paper published in ...", %DIF > 
% which is inaccurate while the ICCAS 2026 proceedings are not yet published. The extended-version note is %DIF > 
% therefore set via \firstnote{} with correct wording; restore \conference{} at production once published. %DIF > 
%\conference{the Proceedings of the 2026 International Conference on Control, Automation and Systems (ICCAS), Sapporo, Japan, 27--30 October 2026} %DIF > 
\firstnote{This paper is an extended version of our paper accepted for presentation at the 2026 International Conference on Control, Automation and Systems (ICCAS), Sapporo, Japan, 27--30 October 2026, and to appear in the conference proceedings. The conference version introduced an isotropic-disk stop-and-scan trigger; the present work replaces the disk model with an occlusion-aware ray-cast visibility model, adds a controlled ablation and a parameter sensitivity analysis, and reports a fully autonomous on-hardware disk-versus-visibility comparison on a mobile platform with per-run survey-grade offline registration of the acquired scans.} %DIF > 
%DIF -------

% Abstract (Do not insert blank lines, i.e. \\) 
%DIF 117c119
%DIF < \abstract{Automated 3D mapping with a survey-grade terrestrial laser scanner requires the platform to remain stationary for minutes per scan, so the environment must be covered with as few scans as possible. Where to stop hinges on a coverage model predicting what a pose will observe. The conventional isotropic-disk model counts every free cell within sensing range as covered---including cells behind walls---and therefore skips scans that genuine observation requires. We replace the disk with a ray-cast visibility region admitting only cells in direct line of sight, define a line-of-sight (LOS) coverage metric over mapped free space, and drive a marginal-gain scan/skip rule embedded in frontier exploration. In a controlled paired ablation in simulation, the visibility rule raises LOS coverage from about 77\% to 84--85\% for one to two additional scans, and a live multi-room run improves coverage from 46.9\% to 95.1\%. In a fully autonomous on-hardware comparison in an industrial test room, with each rule driving a mobile platform carrying a Leica BLK360 G1, the visibility rule raised LOS coverage from 77.6\% to 92.1\%; every visibility run's scans registered offline at survey grade (4--5~mm bundle error, 84--88\% overlap), whereas disk runs yielded at best a single-link network and once a single unregistrable scan. Occlusion-aware visibility, rather than Euclidean proximity, is the appropriate basis for stop-and-scan placement.}
%DIF -------
\abstract{Automated three-dimensional (3D) mapping with a survey-grade terrestrial laser scanner (TLS) is the canonical instance of robotic stop-and-scan light detection and ranging (LiDAR) mapping: the mobile platform must remain stationary for minutes per scan, so the environment must be covered with as few scans as possible. Where to stop hinges on a coverage model predicting what a scan pose will observe. The conventional isotropic-disk model counts every free cell within sensing range as covered---including cells behind walls---and therefore skips scans that genuine observation requires. We replace the disk with a ray-cast visibility region computed on the robot's live two-dimensional (2D) occupancy map, admitting only cells in direct line of sight; define a line-of-sight (LOS) coverage metric over mapped free space; and drive a marginal-gain scan/skip rule embedded in frontier exploration. The planner thus performs 2D scan-station placement for subsequent 3D mapping. In a controlled paired ablation in simulation, the visibility rule raises LOS coverage from about 77\% to 84--85\% for one to two additional scans, and it also outperforms a disk baseline governed by the identical marginal-gain rule, isolating the coverage model itself as the decisive factor. In a fully autonomous on-hardware comparison in an industrial test room, with each rule driving a mobile platform carrying a Leica BLK360 G1, the visibility rule raised LOS coverage from 77.6\% to 92.1\%; every visibility run's scans registered offline at survey grade (4--5~mm bundle error, 84--88\% overlap), whereas disk runs yielded at best a single-link network and once a single unregistrable scan. The results indicate that, for the room-scale indoor environments studied, occlusion-aware visibility is a sounder basis than Euclidean proximity for stop-and-scan placement.} %DIF > 
%DIF -------

% Keywords
%DIF 120c122
%DIF < \keyword{terrestrial laser scanner; stop-and-scan; visibility region; line-of-sight coverage; next-best-view; frontier exploration; 3D mapping; occlusion-aware planning}
%DIF -------
\keyword{terrestrial laser scanner; LiDAR; stop-and-scan; visibility region; line-of-sight coverage; next-best-view; frontier exploration; 3D mapping; occlusion-aware planning} %DIF > 
%DIF -------
...
\providecommand{\DIFdel}[1]{{\protect\color{red}\protect\scriptsize\protect\sout{#1}}} %DIF PREAMBLE
\providecommand{\DIFdelNS}[1]{{\protect\color{red}\protect\scriptsize #1}} %DIF PREAMBLE
...
growing\DIFdelbegin \DIFdel{: a dense , }\DIFdelend \DIFaddbegin \DIFadd{. A dense and }\DIFaddend accurate point cloud is becoming an infrastructural
asset\DIFdelbegin \DIFdel{that
}\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend downstream robots localize against \DIFaddbegin \DIFadd{it}\DIFaddend , plan collision-free motion within
\DIFaddbegin \DIFadd{it}\DIFaddend , and reason over \DIFaddbegin \DIFadd{it}\DIFaddend . This raises the practical question of how such maps can be acquired
\emph{automatically} rather than through labor-intensive manual scanning. \DIFaddbegin \DIFadd{The
...
stationary sensors. \DIFaddbegin \DIFadd{We refer to this regime as \emph{robotic stop-and-scan
...
are aligned offline rather than registered incrementally in real time. }\DIFaddend A
terrestrial laser scanner (TLS) \DIFdelbegin \DIFdel{such as }\DIFdelend \DIFaddbegin \DIFadd{is the canonical instance of such a sensor; }\DIFaddend the
Leica BLK360 G1~\cite{leica-blk360} \DIFaddbegin \DIFadd{used in our experiments }\DIFaddend acquires a dense
\DIFdelbegin \DIFdel{, colorized point cloud and a panoramic
image }\DIFdelend \DIFaddbegin \DIFadd{point cloud }\DIFaddend from a single viewpoint \DIFdelbegin \DIFdel{, }\DIFdelend with a 3D point accuracy on the order of
millimeters and a full-dome scan time of a few minutes. \DIFdelbegin \DIFdel{Two properties of this
device }\DIFdelend \DIFaddbegin \DIFadd{(The scanner also
...
properties }\DIFaddend shape the mapping problem. First, the \DIFdelbegin \DIFdel{scanner }\DIFdelend \DIFaddbegin \DIFadd{sensor }\DIFaddend must remain completely
...
aligned offline. \DIFdelbegin \DIFdel{Conventional mobile mapping, which assumes continuous
acquisition }\DIFdelend \DIFaddbegin \DIFadd{We emphasize that these two properties characterize the
survey-grade stop-and-scan class, not 3D laser scanning in general: continuous
mobile-mapping scanners acquire }\DIFaddend under motion and \DIFdelbegin \DIFdel{incremental online
registration, }\DIFdelend \DIFaddbegin \DIFadd{register incrementally online
through simultaneous localization and mapping (SLAM), trading survey-grade
accuracy for acquisition speed. SLAM-based continuous mobile mapping }\DIFaddend therefore
does not transfer directly to \DIFdelbegin \DIFdel{this \emph{stop-and-scan} workflow .
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The same workflow is shared by a broad class of high-precision stationary
sensors beyond TLS}\DIFdelend \DIFaddbegin \DIFadd{the stop-and-scan workflow addressed here. The method developed in this
...
\DIFadd{A broader class of sensors shares this workflow in that they require
stationary acquisition or a dwell time at each viewpoint}\DIFaddend , including
structured-light 3D scanners, thermal panorama units, ground-penetrating radar
\DIFaddbegin \DIFadd{(GPR)}\DIFaddend , gas and spectral sensors, and non-destructive testing
probes\DIFdelbegin \DIFdelNS{, all of which require either stationary acquisition or a dwell
time at each viewpoint~\cite{stopandgo-review,gas-coverage,robotic-ndt}}\DIFdelend \DIFaddbegin \DIFadd{~\cite{stopandgo-review,gas-coverage,robotic-ndt}. These instruments
...
modalities such as GPR (Section~\ref{sec:discussion})}\DIFaddend . Whenever
...
cell within the sensing range as \DIFdelbegin \DIFdel{covered}\DIFdelend \DIFaddbegin \DIFadd{covered---a sensor-footprint approximation
widely used to drive online decisions in frontier exploration and coverage
practice~\cite{yamauchi1997,umari2017,choset2001,galceran2013,placed2023}}\DIFaddend . This assumption is reasonable in open
...
In this paper we \DIFdelbegin \DIFdel{argue that }\DIFdelend \DIFaddbegin \DIFadd{develop a coverage model and placement policy built on
}\DIFaddend occlusion-aware visibility, \DIFdelbegin \DIFdel{rather }\DIFdelend \DIFaddbegin \DIFadd{and we evaluate whether it provides a sounder basis
for stop-and-scan placement }\DIFaddend than Euclidean proximity\DIFdelbegin \DIFdel{, is the appropriate basis for stop-and-scan placement , and we develop
a coverage model and placement policy built on that principle.
We }\DIFdelend \DIFaddbegin \DIFadd{. The study is organized
around one principal research question:
}\begin{quote}
\DIFadd{\textbf{RQ.} \emph{In online robotic exploration of an unknown indoor
...
\DIFadd{Three subquestions structure the investigation: (RQ1)~how should per-pose
...
\DIFadd{Concretely, we }\DIFaddend replace the
...
scan pose\DIFaddbegin \DIFadd{; both are computed on the live 2D occupancy grid, so the method
performs 2D scan-station planning for subsequent 3D mapping
(Section~\ref{sec:scope2d})}\DIFaddend . The visibility region drives a marginal-gain scan/skip rule that is
...
sensing, \DIFaddbegin \DIFadd{defined on the live 2D occupancy grid, }\DIFaddend with a conservative LOS
coverage metric \DIFdelbegin \DIFdel{defined }\DIFdelend over mapped free space (Section~\ref{sec:coverage});
...
model from exploration stochasticity \DIFaddbegin \DIFadd{and---via a disk baseline run under the
identical marginal-gain rule---from the acceptance rule}\DIFaddend , together with a
...
This work extends a preliminary conference \DIFdelbegin \DIFdel{manuscript}\DIFdelend \DIFaddbegin \DIFadd{paper}\DIFaddend ~\cite{iccas2026}, \DIFdelbegin \DIFdel{currently
under review }\DIFdelend \DIFaddbegin \DIFadd{accepted
for presentation }\DIFaddend at ICCAS 2026, \DIFdelbegin \DIFdel{that used }\DIFdelend \DIFaddbegin \DIFadd{and the overlap between the two is as
follows. The conference version contributes the stop-and-scan system
integration (frontier exploration coupled to a scan sequencer) and }\DIFaddend an
isotropic-disk \DIFdelbegin \DIFdel{trigger; here the disk is replaced by the }\DIFdelend \DIFaddbegin \DIFadd{scan trigger, demonstrated in simulation. New to this article
are: the }\DIFaddend ray-cast visibility \DIFdelbegin \DIFdel{model, the controlled ablation and parameter analysis are new, and a }\DIFdelend \DIFaddbegin \DIFadd{coverage model and the LOS coverage metric
...
study---the }\DIFaddend fully autonomous on-hardware disk-versus-visibility comparison \DIFdelbegin \DIFdel{with survey-grade
}\DIFdelend \DIFaddbegin \DIFadd{and
the per-run }\DIFaddend offline registration of the acquired \DIFdelbegin \DIFdel{BLK360 scans
is added to complement the simulation results. }\DIFdelend \DIFaddbegin \DIFadd{scans
(Sections~\ref{sec:fieldsim} and~\ref{sec:realhw}). No figure, table, or
quantitative result is shared between the two manuscripts.
}\DIFaddend 

\DIFaddbegin \DIFadd{The remainder of this article is organized as follows.
...
shaping~\cite{he2025,quin2021}\DIFaddbegin \DIFadd{, incremental frontier structures with
hierarchical planning~\cite{zhou2021}, and sampling-based online informative
path planning~\cite{schmid2020}}\DIFaddend . Most of these methods, however, assume a sensor
...
surface, and they have been surveyed \DIFdelbegin \DIFdel{both }\DIFdelend for automated 3D reconstruction and
inspection~\cite{scott2003}\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend for sensor planning in computer
vision~\cite{tarabanis1995}\DIFaddbegin \DIFadd{, and, more recently, for view planning in robot
active vision at large~\cite{zeng2020}}\DIFaddend . Coverage path planning, which instead seeks a path
...
Survey-grade terrestrial scanning\DIFaddbegin \DIFadd{, as practiced with the tripod-class
instruments considered here, }\DIFaddend departs from the continuous-acquisition
assumption: each measurement requires a stationary dwell of minutes and
registration is performed offline~\cite{stopandgo-review}. \DIFaddbegin \DIFadd{(Scanners that
relax these constraints exist, but at accuracies below the survey grade
targeted here.) }\DIFaddend Prior automated TLS
...
Varshosaz\DIFaddbegin \DIFadd{~\cite{mozaffar2016occlusion} }\DIFaddend optimize scanner placement to reduce
occlusions\DIFdelbegin \DIFdel{~\cite{mozaffar2016occlusion}}\DIFdelend , and Prieto et al.\DIFaddbegin \DIFadd{~\cite{prieto2017nbs} }\DIFaddend drive a next-best-scan
policy with a line-of-sight visibility analysis\DIFdelbegin \DIFdel{~\cite{prieto2017nbs}.}\DIFdelend \DIFaddbegin \DIFadd{. Recent formulations cast
...
route between them with enforced network redundancy. }\DIFaddend Crucially, however, these
methods operate \emph{offline on a prior model} of the environment (a \DIFdelbegin \DIFdel{BIM}\DIFdelend \DIFaddbegin \DIFadd{building
information model (BIM)}\DIFaddend , floor plan, or an initial coarse scan), over which
...
proximity-based: an isotropic-disk or fixed-spacing rule, \DIFdelbegin \DIFdel{which is the de facto
choice }\DIFdelend \DIFaddbegin \DIFadd{a commonly used
online treatment }\DIFaddend in stop-and-go practice, over-claims coverage behind walls and
suppresses necessary scans. Replacing that rule with a ray-cast visibility model is the
...
Let the environment be \DIFaddbegin \DIFadd{represented by }\DIFaddend a two-dimensional occupancy
grid~\cite{elfes1989,thrun2005} $\mathcal{M}$ produced online by SLAM,
following the \texttt{nav\_msgs/OccupancyGrid}
convention\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. The grid is an $H\times W$ array of square cells of side $\rho$,
the grid resolution ($\rho=0.05$~m in all experiments); }\DIFaddend each cell $c$ \DIFaddbegin \DIFadd{is
...
cell }\DIFaddend carries an integer value $o(c)$, where $o(c)=-1$ denotes unmapped
(unknown) space and $o(c)\in[0,100]$ is \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a scaled }\DIFaddend occupancy probability. Cells
are classified as
...
with thresholds \DIFaddbegin \DIFadd{ordered as $0\le\theta_{\mathrm{free}}<
\theta_{\mathrm{occ}}\le100$; we use }\DIFaddend $\theta_{\mathrm{free}}=25$ and
$\theta_{\mathrm{occ}}=65$\DIFdelbegin \DIFdel{. The
two }\DIFdelend \DIFaddbegin \DIFadd{, the long-standing free/occupied convention of the
...
yield the same classification. The two }\DIFaddend thresholds are deliberately separated: a
cell is treated as free only when its occupancy is confidently low
(\DIFdelbegin \DIFdel{$\le 25$}\DIFdelend \DIFaddbegin \DIFadd{$\le\theta_{\mathrm{free}}$}\DIFaddend ) and as an obstacle only when confidently high
(\DIFdelbegin \DIFdel{$\ge 65$}\DIFdelend \DIFaddbegin \DIFadd{$\ge\theta_{\mathrm{occ}}$}\DIFaddend ); the intermediate band is treated as unknown so
...
$\mathcal{F}$ is genuinely observed. \DIFaddbegin \DIFadd{Throughout the paper, $s_i$ denotes an
...
\caption{\DIFaddFL{Coverage models for a single scan pose ($R=6$~m).
...
\DIFaddend \subsection{Ray-Cast Visibility Region (Proposed)}
...
\DIFaddbegin \DIFadd{Note that the set definition above characterizes the ideal geometric
...
\DIFaddbegin \subsection{\DIFadd{Scope: 2D Station Planning for 3D Mapping}}
\label{sec:scope2d}

\DIFadd{The visibility model and the LOS metric are deliberately two-dimensional: they
...
\DIFaddend \section{Visibility-Based Stop-and-Scan Placement}
\label{sec:placement}
...
carries a large fixed cost (\DIFdelbegin \DIFdel{a single BLK360 G1 colorized scan takes on the order
of two to three minutes }\DIFdelend \DIFaddbegin \DIFadd{minutes per acquisition for the survey-grade
scanner used in our experiments; Section~\ref{sec:setup}}\DIFaddend ), the placement
policy must take as few scans as possible while \DIFdelbegin \DIFdel{still guaranteeing }\DIFdelend \DIFaddbegin \DIFadd{maintaining high }\DIFaddend LOS coverage
of the \DIFaddbegin \DIFadd{mapped }\DIFaddend free space.

\subsection{Marginal Visibility Gain}

As the robot travels, a scan candidate is raised every $d$ metres of \DIFaddbegin \DIFadd{net
}\DIFaddend straight-line displacement in the map frame from the \DIFdelbegin \DIFdel{most recent scan pose
(net
displacement, not }\DIFdelend \DIFaddbegin \DIFadd{previous candidate pose
(the evaluation reference, initialized to the start pose and advanced at every
raised candidate whether or not it is accepted; net displacement rather than
}\DIFaddend path length, so dwelling or circling in place raises no candidate). Let $S=\{s_1,\dots,s_m\}$ be the scans accepted so far and
$C(S)=\bigcup_i B(s_i,R)$ their union coverage. \DIFaddbegin \DIFadd{The visibility regions of
...
a single reference map; Section~\ref{sec:setup}). }\DIFaddend For a candidate pose \DIFdelbegin \DIFdel{$c$}\DIFdelend \DIFaddbegin \DIFadd{$p$}\DIFaddend ,
...
\emph{\DIFaddbegin \DIFadd{normalized }\DIFaddend marginal gain}

\begin{equation}
g(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend \mid S)=\DIFdelbegin \DIFdel{\frac{\lvert B(c,R)\setminus C(S)\rvert}{\lvert B(c,R)\rvert}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\lvert B(p,R)\setminus C(S)\rvert}{\lvert B(p,R)\rvert}}\DIFaddend ,
\qquad g\in[0,1],
\end{equation}
...
\begin{equation}
a(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend \mid S)=\lvert B(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend ,R)\setminus C(S)\rvert\cdot\rho^2,
\end{equation}

where \DIFaddbegin \DIFadd{cells are squares of side }\DIFaddend $\rho$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend the grid resolution,
\DIFdelbegin \DIFdel{so }\DIFdelend \DIFaddbegin \DIFadd{$\rho=0.05$~m), so each cell contributes area $\rho^2$ and }\DIFaddend $a$ is in
m\textsuperscript{2}. Both \DIFdelbegin \DIFdel{are
}\DIFdelend \DIFaddbegin \DIFadd{quantities are defined only for a nonempty
visibility region $B(p,R)$, and are }\DIFaddend evaluated directly on the visibility
regions of Section~\ref{sec:coverage}, so occlusion is respected: area behind
a wall is never part of \DIFdelbegin \DIFdel{$B(c,R)$ }\DIFdelend \DIFaddbegin \DIFadd{$B(p,R)$ }\DIFaddend and never inflates either the coverage or the
...
is rejected outright \DIFdelbegin \DIFdel{and raises no scan}\DIFdelend \DIFaddbegin \DIFadd{before the gain is evaluated, so the denominator of the
normalized gain never vanishes}\DIFaddend .
...
of a \DIFdelbegin \DIFdel{terrestrial scanner}\DIFdelend \DIFaddbegin \DIFadd{room-scale stop-and-scan sensor}\DIFaddend : because $R$ is comparable to the size of a
room, the region \DIFdelbegin \DIFdel{$B(c,R)$ }\DIFdelend \DIFaddbegin \DIFadd{$B(p,R)$ }\DIFaddend is large, so a small but genuinely occluded pocket contributes
...
\begin{equation}
\textsf{scan}(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend )\iff
\big(g(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend \mid S)\ge\tau\big)\ \lor\ \big(a(\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend \mid S)\ge A_{\min}\big),
\end{equation}
...
\DIFdelbegin \DIFdel{For comparison, the
isotropic-disk policy reduces to skipping }\DIFdelend \DIFaddbegin 

\DIFadd{Two disk-based baselines are evaluated against this rule. The first is the
\emph{proximity-spacing rule} of stop-and-go practice: skip }\DIFaddend any candidate
within $R$ (\DIFdelbegin \DIFdel{straight-line}\DIFdelend \DIFaddbegin \DIFadd{straight line}\DIFaddend ) of an existing scan\DIFdelbegin \DIFdel{, the baseline against which the visibility ruleis evaluated}\DIFdelend \DIFaddbegin \DIFadd{. We note explicitly that this
...
difference is the coverage model}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \While{exploration not converged}{
%DIFDELCMD <   drive toward frontier goal on $\mathcal{M}$\;
%DIFDELCMD <   \If{$\lVert \mathrm{pose} - q \rVert_2 \ge d$}{
%DIFDELCMD <     $c \leftarrow \mathrm{pose}$;\quad $q \leftarrow c$\;
%DIFDELCMD <     $B(c,R) \leftarrow \textsc{RayCastVisibility}(\mathcal{M}, c, R)$\;
%DIFDELCMD <     \If{$\lvert B(c,R)\rvert \cdot \rho^2 < 0.5$}{
%DIFDELCMD <       \textbf{continue} \tcp*{degenerate pose: skip}
%DIFDELCMD <     }
%DIFDELCMD <     $g \leftarrow \lvert B(c,R)\setminus C(S)\rvert / \lvert B(c,R)\rvert$\;
%DIFDELCMD <     $a \leftarrow \lvert B(c,R)\setminus C(S)\rvert \cdot \rho^2$\;
%DIFDELCMD <     \If{$g \ge \tau$ \textbf{\emph{or}} $a \ge A_{\min}$}{
%DIFDELCMD <       stop and acquire stationary scan at $c$\;
%DIFDELCMD <       $S \leftarrow S \cup \{c\}$\;
%DIFDELCMD <     }
%DIFDELCMD <   }
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \While{exploration not converged}{
...
\DIFaddend \tcp{Coverage-completion phase (optional)}
\DIFdelbegin %DIFDELCMD < \While{$\exists$ pocket $P \subseteq \mathcal{F}\setminus C(S)$ with $\mathrm{area}(P) \ge A_{\mathrm{pocket}}$ and $P$ reachable}{
%DIFDELCMD <   navigate into $P$;\quad acquire scan at reached pose $c$\;
%DIFDELCMD <   $S \leftarrow S \cup \{c\}$\;
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \While{$\exists$ pocket $P \subseteq \mathcal{F}\setminus C(S)$ with $\mathrm{area}(P) \ge A_{\mathrm{pocket}}$ and $P$ reachable}{
...
\DIFdelbegin \DIFdel{Figure~}\DIFdelend \DIFaddbegin \DIFadd{All navigation is subject to the platform's physical constraints: Nav2 plans on
an inflated costmap, so every path and every scan candidate keeps the robot
footprint (Figure~\ref{fig:redbot}b) clear of obstacles by the Nav2 costmap
inflation margin (inflation radius $0.70$~m with a $0.22$~m robot radius in
all reported runs).
Figure~}\DIFaddend \ref{fig:arch} shows how these components connect: the frontier
...
\caption{System architecture. The \DIFdelbeginFL \DIFdelFL{frontier }\DIFdelendFL exploration layer (\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top}\DIFaddendFL ) proposes \DIFdelbeginFL \DIFdelFL{and
orders }\DIFdelendFL navigation
goals on the live \DIFdelbeginFL \DIFdelFL{2D }\DIFdelendFL occupancy grid\DIFdelbeginFL \DIFdelFL{. The }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; the }\DIFaddendFL stop-and-scan sequencer (\DIFdelbeginFL \DIFdelFL{right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bottom}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{raises a candidate at each distance-gate interval and, using
the occlusion-aware ray-cast visibility region $B(s,R)$ (highlighted), }\DIFdelendFL applies
the \DIFdelbeginFL \DIFdelFL{marginal-gain }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{visibility-based }\DIFaddendFL scan/skip rule \DIFaddbeginFL \DIFaddFL{(highlighted}\DIFaddendFL ; \DIFdelbeginFL \DIFdelFL{accepted poses yield }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{this work) and triggers
}\DIFaddendFL stationary \DIFdelbeginFL \DIFdelFL{survey-grade
}\DIFdelendFL scans that are registered offline.\DIFdelbeginFL \DIFdelFL{The visibility coverage model is the
contribution of this work; the exploration layer is an existing component.}\DIFdelendFL }
\label{fig:arch}
\end{figure}
...
and executes its \DIFdelbegin \DIFdel{colorized-scan }\DIFdelend \DIFaddbegin \DIFadd{full-dome scan }\DIFaddend workflow, and we verified that the device can be
...
stationary sensor (Figure~\ref{fig:redbot}). \DIFdelbegin \DIFdel{This is the mobile platform whose absence was noted in
the conference version; its availability enables the
real-hardware experiments reported below}\DIFdelend \DIFaddbegin \DIFadd{The deck places the scanner's
...
\begin{figure}[H]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.72\linewidth]{figures/fig9_redbot04_platform.jpg}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{figures/fig9_redbot04_platform_annot.png}
\DIFaddendFL \caption{RedBOT04 \DIFdelbeginFL \DIFdelFL{, the four-wheeled }\DIFdelendFL mobile scan platform\DIFdelbeginFL \DIFdelFL{used for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. (\textbf{a})~Annotated view with }\DIFaddendFL the
\DIFdelbeginFL \DIFdelFL{real-hardware experiments, carrying a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{deck-mounted }\DIFaddendFL Leica BLK360 G1\DIFdelbeginFL \DIFdelFL{terrestrial laser scanner
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{black unit, center}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{b}}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{as its survey-grade stationary sensor}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{~top-view footprint schematic with
the principal dimensions (cm)}\DIFaddendFL .\DIFdelbeginFL \DIFdelFL{The platform
additionally carries a 2D LiDAR and an RGB-D camera for Cartographer SLAM and
Nav2 navigation.}\DIFdelendFL }
\label{fig:redbot}
\end{figure}
...
pronounced. For the real-hardware experiments (Sections~\DIFdelbegin \DIFdel{\ref{sec:realhw}
and~\ref{sec:fieldsim}}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:fieldsim}
and~\ref{sec:realhw}}\DIFaddend ), a physical industrial test room is used\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. It is }\DIFaddend an open
exhibition and staging space \DIFdelbegin \DIFdel{(Figure~\ref{fig:testroom}) }\DIFdelend with a flat\DIFdelbegin \DIFdel{reflective floor, }\DIFdelend \DIFaddbegin \DIFadd{, reflective floor; }\DIFaddend a display wall \DIFaddbegin \DIFadd{of
monitors and product panels runs }\DIFaddend along one side, and freestanding equipment
\DIFdelbegin \DIFdel{and furniture that }\DIFdelend \DIFaddbegin \DIFadd{cases and furniture }\DIFaddend create genuine self-occlusion\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{. Floor markings indicate the
survey reference layout. The room is }\DIFaddend representative of the factory and
demonstration environments the platform targets\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and it is shown in
...
reference map.
}\DIFaddend The figure of merit is LOS coverage (Section~\ref{sec:los}). Because
...
the fully autonomous on-hardware placement comparison
}\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{Section}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Sections}\DIFaddendFL ~\ref{sec:fieldsim} \DIFdelbeginFL \DIFdelFL{) }\DIFdelendFL and\DIFdelbeginFL \DIFdelFL{the per-run offline registration of the
acquired scans (Section}\DIFdelendFL ~\ref{sec:realhw}).\DIFdelbeginFL \DIFdelFL{The open exhibition space has a reflective floor, a
display wall of monitors and product panels along one side, and freestanding
equipment cases, producing genuine self-occlusion. Floor markings indicate the
survey reference layout.}\DIFdelendFL }
\label{fig:testroom}
\end{figure}
...
\DIFdelbeginFL \DIFdelFL{Obstacle }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Free-space }\DIFaddendFL dilation radius   & env.-tuned & Morphological dilation \DIFdelbeginFL \DIFdelFL{enforcing a safety clearance margin}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of mapped free space during decision-map construction, before frontier extraction}\DIFaddendFL . \\
...
We evaluate the visibility-based policy \DIFdelbegin \DIFdel{against the isotropic-disk baseline }\DIFdelend on two axes, LOS coverage and
stationary scan count, organizing the evaluation so that the most rigorous
evidence carries the argument. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Four placement policies are compared throughout:
...
\emph{ray-cast visibility} policy. The disk baselines represent a commonly
used proximity-based online coverage treatment in stop-and-go practice
...
map of an unknown environment. The }\DIFaddend simulation study comes first;
...
\DIFdelbegin \DIFdel{BLK360 }\DIFdelend scans acquired live, and Section~\ref{sec:realhw} closes the loop by
...
matches the \DIFdelbegin \DIFdel{room-scale dimensions }\DIFdelend \DIFaddbegin \DIFadd{local room/bay scale, and hence the occlusion scale, }\DIFaddend of the indoor
environments targeted here\DIFdelbegin \DIFdel{,
which are rarely much larger than $6$~m across}\DIFdelend .

\begin{figure}[H]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.85\linewidth]{figures/fig1_concept.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Coverage model for a single BLK360 scan pose ($R=6$~m). (\textbf{a})~The
isotropic disk $B_{\mathrm{disk}}(s,R)$ counts every cell within $R$ as covered,
bleeding through partition and perimeter walls. (\textbf{b})~The proposed
ray-cast visibility region $B(s,R)$ stops at the first occupied or unknown cell
along each ray, so only genuine line-of-sight area is claimed.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:concept}
...
%DIFDELCMD < \centering
%DIFDELCMD < %%%
\DIFdelendFL \includegraphics[width=0.7\linewidth]{figures/fig2_singleroom_sota.png}
\caption{Proposed visibility stop-and-scan on the single-room world
($R=6$~m): 4~scans, $97.9\%$ LOS coverage. \DIFdelbeginFL \DIFdelFL{The map, robot path, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Each accepted scan pose (marker)
}\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{per-scan }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{its }\DIFaddendFL visibility \DIFdelbeginFL \DIFdelFL{polygons (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{polygon share one color, indexed by }\DIFaddendFL scan order
\DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $\#1\dots\#4$)\DIFdelbeginFL \DIFdelFL{are overlaid on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{Cartographer }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{grayscale background is the }\DIFaddendFL occupancy map \DIFaddbeginFL \DIFaddFL{(light gray
free, black occupied, darker gray unknown) and the blue line is the robot
path}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The same color convention is used in all placement figures.}\DIFaddendFL }
...
only in which candidates they accept (Figure~\ref{fig:ablation}). \DIFaddbegin \DIFadd{For the
controlled replay only, each logged trajectory is resampled at fixed $2$~m
path-length intervals to construct the common candidate sequence: path-length
resampling of a logged trajectory yields a deterministic sequence shared by all
policies, and the interval is chosen denser than the live $d=3$~m
net-displacement trigger so that no policy is starved of decision opportunities
relative to a live run. }\DIFaddend This
...
\DIFaddbegin \DIFadd{spacing }\DIFaddend rule is structurally capped: once two scans are placed, almost every
later candidate lies within $R$ (\DIFdelbegin \DIFdel{straight-line}\DIFdelend \DIFaddbegin \DIFadd{straight line}\DIFaddend ) of a prior scan, so it stops
scanning even though walls occlude much of the area it counts as covered.

\DIFaddbegin \DIFadd{The disk marginal-gain baseline isolates the coverage model from the
...
\caption{Controlled skip-rule ablation \DIFdelbeginFL \DIFdelFL{: }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on an }\DIFaddendFL identical map and \DIFdelbeginFL \DIFdelFL{identical }\DIFdelendFL candidate
path\DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL only the skip decision differs. (\textbf{a},\textbf{b})~\DIFdelbeginFL \DIFdelFL{single-room }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Single-room }\DIFaddendFL and
(\textbf{c},\textbf{d})~multi-room, \DIFdelbeginFL \DIFdelFL{each }\DIFdelendFL under the disk \DIFaddbeginFL \DIFaddFL{spacing }\DIFaddendFL rule
(\textbf{a},\textbf{c}) and the visibility rule (\textbf{b},\textbf{d}).\DIFdelbeginFL \DIFdelFL{The disk
rule is structurally capped at 2--3 scans and leaves occluded pockets/rooms
($\approx 74$--$87\%$ LOS); the visibility rule fills them ($94$--$95\%$ LOS).}\DIFdelendFL }
\label{fig:ablation}
\end{figure}
...
\caption{\DIFdelbeginFL \DIFdelFL{Three-way }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Four-way }\DIFaddendFL skip-rule comparison on identical paths ($N=5$ single-room
and $N=10$ multi-room trajectories, paired per path)\DIFdelbeginFL \DIFdelFL{: uniform no-skip reference,
isotropic-disk baseline, and ray-cast visibility (ours)}\DIFdelendFL . (\textbf{a})~LOS
coverage; (\textbf{b})~stationary scan count\DIFdelbeginFL \DIFdelFL{on a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL log scale
(mean\,$\pm$\,s.d.).\DIFdelbeginFL \DIFdelFL{The
visibility rule improves LOS over the disk baseline (single-room
$77.0\to85.4\%$; multi-room $77.0\to84.0\%$) while using only a small fraction of
the uniform reference's scan count ($28.0$/$40.8$ scans).}\DIFdelendFL }
\label{fig:summary}
\end{figure}

\begin{table}[H]
\caption{Controlled skip-rule ablation\DIFdelbeginFL \DIFdelFL{on identical maps and candidate paths}\DIFdelendFL : \DIFdelbeginFL \DIFdelFL{three }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{four }\DIFaddendFL placement policies replayed on
the same fixed candidate sequences ($N=5$ single-room paths; $N=10$ multi-room
paths\DIFdelbeginFL \DIFdelFL{, five per coverage model}\DIFdelendFL ). \DIFdelbeginFL \DIFdelFL{The
only difference between rows is the coverage model used for the scan/skip
decision. }\DIFdelendFL Values are mean\,$\pm$\,s.d. The uniform no-skip policy \DIFdelbeginFL \DIFdelFL{(scan at every
candidate) }\DIFdelendFL is an upper
reference \DIFdelbeginFL \DIFdelFL{: scanning everywhere, it approaches }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{maximum
}\DIFdelendFL LOS attainable along the path\DIFdelbeginFL \DIFdelFL{but }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL at a prohibitive scan count\DIFdelbeginFL \DIFdelFL{. The visibility
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{;
the disk marginal-gain row applies the identical accept/skip }\DIFaddendFL rule \DIFdelbeginFL \DIFdelFL{attains LOS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{as the
visibility policy, so }\DIFaddendFL within \DIFdelbeginFL \DIFdelFL{$2.6$--$6.5$ percentage points of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each environment }\DIFaddendFL that \DIFdelbeginFL \DIFdelFL{brute-force
reference using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pair differs }\DIFaddendFL only \DIFdelbeginFL \DIFdelFL{$3.6$ scans on average, while improving on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL the
\DIFdelbeginFL \DIFdelFL{isotropic
disk baseline by $+8.4$ (single-room) and $+7.0$ (multi-room) percentage points}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{coverage model}\DIFaddendFL .}
\label{tab:ablation}
...
\midrule
\DIFdelbeginFL %DIFDELCMD < \multirow{3}{*}{Single-room} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{4}{*}{Single-room} \DIFaddendFL & Uniform, no skip (reference) & $91.9\pm7.0$ & $28.0\pm13.0$ \\
                             & \DIFdelbeginFL \DIFdelFL{Isotropic disk }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Disk, spacing rule }\DIFaddendFL (baseline) & $77.0\pm4.9$ & $2.0\pm0.0$ \\
                             & \DIFaddbeginFL \DIFaddFL{Disk, marginal-gain rule      }& \DIFaddFL{$77.7\pm3.8$ }& \DIFaddFL{$2.6\pm0.5$ }\\
                             & \DIFaddendFL Ray-cast visibility (ours)   & $\mathbf{85.4\pm6.2}$ & $3.6\pm0.9$ \\
\midrule
\DIFdelbeginFL %DIFDELCMD < \multirow{3}{*}{Multi-room}  %%%
\DIFdelendFL \DIFaddbeginFL \multirow{4}{*}{Multi-room}  \DIFaddendFL & Uniform, no skip (reference) & $86.6\pm9.9$ & $40.8\pm11.8$ \\
                             & \DIFdelbeginFL \DIFdelFL{Isotropic disk }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Disk, spacing rule }\DIFaddendFL (baseline) & $77.0\pm6.4$ & $2.3\pm0.5$ \\
                             & \DIFaddbeginFL \DIFaddFL{Disk, marginal-gain rule      }& \DIFaddFL{$59.1\pm1.3$ }& \DIFaddFL{$2.9\pm0.3$ }\\
                             & \DIFaddendFL Ray-cast visibility (ours)   & $\mathbf{84.0\pm9.2}$ & $3.6\pm0.7$ \\
\bottomrule
\end{tabularx}
\noindent\DIFdelbeginFL %DIFDELCMD < \footnotesize{On paths that traverse every room, the per-path gain of
%DIFDELCMD < the visibility rule over the disk rule reaches $+21$ to $+23$ percentage points;
%DIFDELCMD < the means above are over all paths including partial-traversal paths. All three
%DIFDELCMD < policies traverse the identical candidate path, so total travel distance is the
%DIFDELCMD < same across rows; the policies differ only in scan count, the dominant cost in
%DIFDELCMD < stop-and-scan sensing.}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \footnotesize{On paths that traverse every room, the per-path gain
...
We characterize the two skip thresholds by deterministic replay \DIFaddbegin \DIFadd{of the
visibility policy }\DIFaddend over the same fixed paths as the ablation ($N=5$
single-room, $N=10$ multi-room \DIFdelbegin \DIFdel{) per
environment (}\DIFdelend \DIFaddbegin \DIFadd{paths); }\DIFaddend Figure~\ref{fig:sweep} \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{reports means
with per-path standard deviations as error bars}\DIFaddend . Varying the relative
threshold $\tau$ over $[0.10,0.50]$ changes \DIFdelbegin \DIFdel{both LOS coverage and }\DIFdelend \DIFaddbegin \DIFadd{mean LOS coverage by at most about
two points (single-room $86.4\to84.3\%$; multi-room $85.2\to84.0\%$) and
mean }\DIFaddend scan count by \DIFdelbegin \DIFdel{only about $2$ percentage points}\DIFdelend \DIFaddbegin \DIFadd{less than one scan}\DIFaddend , so $\tau$ lies in a robust, flat
region and $\tau=0.30$ is not a sensitive choice. The absolute threshold
$A_{\min}$ is the operative knob \DIFdelbegin \DIFdel{: it directly }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend trades coverage against scan count\DIFdelbegin \DIFdel{, with its knee at
}\DIFdelend \DIFaddbegin \DIFadd{. We
identify }\DIFaddend $A_{\min}=5$~m\textsuperscript{2} \DIFdelbegin \DIFdel{, where coverage is within about $5$ percentage
}\DIFdelend \DIFaddbegin \DIFadd{as the knee by the following
criterion: it is the largest threshold whose mean LOS remains within about
five }\DIFaddend points of the \DIFdelbegin \DIFdel{much more aggressive }\DIFdelend \DIFaddbegin \DIFadd{most aggressive setting
(}\DIFaddend $A_{\min}=1$~m\textsuperscript{2}\DIFdelbegin \DIFdel{setting
(}\DIFdelend \DIFaddbegin \DIFadd{) in both environments (single-room $85.4$
versus }\DIFaddend $90.7\%$\DIFdelbegin \DIFdel{) while taking only about half as many scans }\DIFdelend \DIFaddbegin \DIFadd{; multi-room $84.0$ versus $85.7\%$) while already halving
the scan count }\DIFaddend ($3.6$ versus $7.2$ \DIFdelbegin \DIFdel{). }\DIFdelend \DIFaddbegin \DIFadd{scans in both environments). Beyond the
...
($85.4\to80.8\%$ and $84.0\to83.1\%$). }\DIFaddend This justifies the operating point
$\tau=0.30$, $A_{\min}=5$~m\textsuperscript{2}\DIFaddbegin \DIFadd{, which was fixed before the
hardware campaign}\DIFaddend .

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig8_param_sweep.png}
\caption{Parameter sweep \DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by }\DIFaddendFL deterministic replay \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on the fixed candidate
paths (mean~$\pm$~s.d.\ over }\DIFaddendFL $N=5$ single-room and $N=10$ multi-room paths).
\DIFaddbeginFL \DIFaddFL{Solid curves: LOS coverage }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{left axis); dashed curves: scan count (right
axis). (}\DIFaddendFL \textbf{a})~$\tau$ sweep at $A_{\min}=5$~m\textsuperscript{2}\DIFdelbeginFL \DIFdelFL{: coverage and scan count are flat
across $\tau\in[0.10,0.50]$. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{;
}\DIFaddendFL (\textbf{b})~$A_{\min}$ sweep at $\tau=0.30$\DIFdelbeginFL \DIFdelNS{:
$A_{\min}$ is the operative coverage-vs-scans knob}\DIFdelendFL , \DIFdelbeginFL \DIFdelNS{with the }\DIFdelendFL knee \DIFdelbeginFL \DIFdelNS{at
$5$~m\textsuperscript{2}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{marked}\DIFaddendFL .}
\label{fig:sweep}
\end{figure}
...
acquires a full-dome \DIFdelbegin \DIFdel{colorized }\DIFdelend stationary scan (Figure~\ref{fig:field}). Each
...
start, are discarded as failed trials rather than policy samples. \DIFaddbegin \DIFadd{The exclusion
criterion is independent of the skip rule: a run is discarded only when frontier
exploration stalls before a room-spanning traverse begins, i.e., before the
placement policy has faced any consequential scan/skip decision. Both discarded
runs stalled in this stage and are therefore uninformative about either skip
rule. }\DIFaddend Because each live run maps a slightly different extent of free space,
...
\caption{Fully autonomous on-hardware \DIFdelbeginFL \DIFdelFL{disk-versus-visibility }\DIFdelendFL comparison in the physical test room
(\DIFdelbeginFL \DIFdelFL{RedBOT04 running frontier exploration $+$ Nav2 $+$
Cartographer live; }\DIFdelendFL representative \DIFdelbeginFL \DIFdelFL{of $N=4$ completed }\DIFdelendFL runs\DIFdelbeginFL \DIFdelFL{), stop-and-scan at
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL $R=6$~m\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL . Light grey\DIFdelbeginFL \DIFdelFL{is
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL mapped free space\DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL dark grey\DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{:
}\DIFaddendFL unmapped\DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL black\DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL occupied; \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL purple line\DIFdelbeginFL \DIFdelFL{is
the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL autonomous robot path\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL colored
markers\DIFdelbeginFL \DIFdelFL{are the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL placed scans with their coverage regions. (\textbf{a})~\DIFdelbeginFL \DIFdelFL{The disk model places few scans and leaves the
occluded bays uncovered}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Disk spacing
rule}\DIFaddendFL ; (\textbf{b})~\DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL visibility \DIFdelbeginFL \DIFdelFL{model scans them}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rule}\DIFaddendFL .}
\label{fig:fieldsim}
\end{figure}
...
\DIFaddbegin \DIFadd{Registration is a post-mission batch step; importing and aligning one run's
...
}\DIFaddend Table~\ref{tab:registration} summarizes the per-run registration networks.
...
converged at \DIFdelbegin \DIFdel{$0.004$}\DIFdelend \DIFaddbegin \DIFadd{$4$}\DIFaddend --\DIFdelbegin \DIFdel{$0.005$~m }\DIFdelend \DIFaddbegin \DIFadd{$5$~mm }\DIFaddend with overall overlaps of $84$--$88\%$, and every
individual link fell within the software's highest quality band (maximum error
\DIFdelbegin \DIFdel{$\le 0.015$~m}\DIFdelend \DIFaddbegin \DIFadd{$\le15$~mm}\DIFaddend ; per-link overlaps $74$--$92\%$, errors \DIFdelbegin \DIFdel{$0.003$}\DIFdelend \DIFaddbegin \DIFadd{$3$}\DIFaddend --\DIFdelbegin \DIFdel{$0.006$~m). For a
scanner whose single-setup 3D point accuracy is on the order of a few
millimeters, a }\DIFdelend \DIFaddbegin \DIFadd{$6$~mm). The
}\DIFaddend $4$--$5$~mm bundle \DIFdelbegin \DIFdel{error is at the level of the sensor noise
floor, i.e., registration adds essentially no error beyond the device itself}\DIFdelend \DIFaddbegin \DIFadd{residuals are of the same order of magnitude as the
scanner's stated single-setup 3D point accuracy of a few millimeters}\DIFaddend .
The registered clouds reconstruct the room---display wall, equipment, and
floor---as a single coherent \DIFdelbegin \DIFdel{colorized }\DIFdelend model: Figure~\ref{fig:regcloud} shows
...
($75$--$85\%$ overlap, \DIFdelbegin \DIFdel{$0.002$~m}\DIFdelend \DIFaddbegin \DIFadd{$2$~mm}\DIFaddend ); the fourth disk run placed only one scan and
...
\begin{table}[H]
\caption{Per-run offline registration of the autonomously acquired \DIFdelbeginFL \DIFdelFL{BLK360 }\DIFdelendFL scans
\DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL Leica Cyclone REGISTER~360 PLUS\DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL BLK Edition\DIFdelbeginFL \DIFdelFL{), }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; }\DIFaddendFL cloud-to-cloud\DIFdelbeginFL \DIFdelFL{alignment
without }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, no }\DIFaddendFL targets,
identical settings\DIFdelbeginFL \DIFdelFL{for all runs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL . Every link of every registered network lies within the
software's highest quality band (maximum error \DIFdelbeginFL \DIFdelFL{$\le 0.015$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\le15$}\DIFaddendFL ~\DIFdelbeginFL \DIFdelFL{m}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mm}\DIFaddendFL ).}
\label{tab:registration}
\newcolumntype{Z}{>{\centering\arraybackslash}X}
\begin{tabularx}{\linewidth}{l l Z Z Z Z}
\toprule
\textbf{Policy} & \textbf{Run} & \textbf{Setups} & \textbf{Links} & \textbf{Overall overlap (\%)} & \textbf{Bundle error (\DIFdelbeginFL \DIFdelFL{m}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mm}\DIFaddendFL )} \\
\midrule
Visibility & 1 & 3 & 3  & 86 & \DIFdelbeginFL \DIFdelFL{0.005 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5 }\DIFaddendFL \\
Visibility & 2 & 6 & 13 & 86 & \DIFdelbeginFL \DIFdelFL{0.005 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5 }\DIFaddendFL \\
Visibility & 3 & 5 & 10 & 84 & \DIFdelbeginFL \DIFdelFL{0.005 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5 }\DIFaddendFL \\
Visibility & 4 & 3 & 3  & 88 & \DIFdelbeginFL \DIFdelFL{0.004 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4 }\DIFaddendFL \\
\midrule
Disk & 1 & 1 & 0 & \multicolumn{2}{c}{not registrable (single setup)} \\
Disk & 2 & 2 & 1 & 75 & \DIFdelbeginFL \DIFdelFL{0.002 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2 }\DIFaddendFL \\
Disk & 3 & 2 & 1 & 85 & \DIFdelbeginFL \DIFdelFL{0.002 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2 }\DIFaddendFL \\
Disk & 4 & 2 & 1 & 84 & \DIFdelbeginFL \DIFdelFL{0.002 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2 }\DIFaddendFL \\
\bottomrule
\end{tabularx}
\end{table}
...
visibility run~3 \DIFdelbeginFL \DIFdelFL{, rendered }\DIFdelendFL along two diagonal \DIFdelbeginFL \DIFdelFL{viewing }\DIFdelendFL directions\DIFdelbeginFL \DIFdelFL{. (\textbf{Left})~Toward }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: toward }\DIFaddendFL the display wall
\DIFdelbeginFL \DIFdelFL{with its monitor and product-panel arrays. }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{\textbf{Right}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{left}}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{~Toward
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and toward }\DIFaddendFL the equipment \DIFdelbeginFL \DIFdelFL{and workbench }\DIFdelendFL side \DIFdelbeginFL \DIFdelFL{of the room}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(\textbf{right})}\DIFaddendFL . Red markers
and green edges are the scan stations and \DIFdelbeginFL \DIFdelFL{cloud-to-cloud }\DIFdelendFL links of
Table~\ref{tab:registration}\DIFdelbeginFL \DIFdelFL{; the
five autonomously acquired scans fuse into a single coherent survey-grade model
of the room}\DIFdelendFL .}
\label{fig:regcloud}
\end{figure}
...
two additional scans per environment\DIFaddbegin \DIFadd{; the disk baseline run under the identical
...
the acceptance rule}\DIFaddend . The effect is structural rather than incidental:
...
Several limitations frame the result. First, \DIFdelbegin \DIFdel{LOS coverage is }\DIFdelend \DIFaddbegin \DIFadd{the planning representation is
...
Second, even within its 2D scope, LOS coverage is }\DIFaddend a geometric proxy
...
natural refinement. \DIFdelbegin \DIFdel{Second}\DIFdelend \DIFaddbegin \DIFadd{Third}\DIFaddend , the controlled paired ablation is conducted in
...
\DIFdelbegin \DIFdel{Third}\DIFdelend \DIFaddbegin \DIFadd{Fourth}\DIFaddend , the visibility model presently assumes an optical line-of-sight sensor,
which is
...
We presented an occlusion-aware visibility coverage model for \DIFaddbegin \DIFadd{robotic
}\DIFaddend stop-and-scan 3D \DIFdelbegin \DIFdel{mapping with a terrestrial laser scanner, and a }\DIFdelend \DIFaddbegin \DIFadd{LiDAR mapping, together with a }\DIFaddend marginal-gain placement policy
built on it\DIFaddbegin \DIFadd{; the model performs 2D scan-station planning on the live occupancy
map for subsequent 3D mapping, and a survey-grade terrestrial laser scanner is
its experimental instantiation}\DIFaddend . Replacing the isotropic disk with a ray-cast
...
}\DIFdelend \DIFaddbegin \DIFadd{while a disk baseline governed by the identical marginal-gain rule performed no
...
model itself; }\DIFaddend a live multi-room run improved coverage from $46.9\%$ to
$95.1\%$ relative to the disk model\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend an optional coverage-completion phase
...
via sensor-level semantic descriptions, \DIFaddbegin \DIFadd{extend the coverage model to a
genuinely 3D visibility representation (voxel-, octree-, or surface-based)
that captures vertical occlusion, }\DIFaddend and add a downstream navigation validation
in which an independent robot autonomously localizes against the completed
map. \DIFaddbegin \DIFadd{Within the room-scale indoor environments and the optical stop-and-scan
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \section{\DIFdel{Patents}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend %DIF >  Patents section removed (no patents resulting from this work).

\DIFdelbegin \DIFdel{This section is not mandatory, but may be added if there are patents resulting from the work reported in this manuscript.
}%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \informedconsent{Not applicable.
%DIFDELCMD < 

%DIFDELCMD < Written informed consent for publication must be obtained from participating patients who can be identified (including by the patients themselves). Please state ``Written informed consent has been obtained from the patient(s) to publish this paper'' if applicable.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \informedconsent{Not applicable.}
\DIFaddend 
...
%DIFDELCMD < TLS & Terrestrial laser scanner\\
%DIFDELCMD < LOS & Line of sight\\
%DIFDELCMD < NBV & Next-best-view\\
%DIFDELCMD < SLAM & Simultaneous localization and mapping\\
%DIFDELCMD < MPPI & Model predictive path integral
...
\bibitem{iccas2026}
Kim, S.; \DIFdelbegin \DIFdel{Choi, J.-H.; An, Y.-C.; }\DIFdelend Kuc, T.-Y. Coverage-Aware Stop-and-Scan Frontier Exploration for Automated Survey-Grade 3D Mapping. \DIFdelbegin \DIFdel{Submitted to the }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend \textit{\DIFaddbegin \DIFadd{Proceedings of the }\DIFaddend 2026 International Conference on Control, Automation and Systems (ICCAS)}, Sapporo, Japan, 27--30 October 2026; \DIFdelbegin \DIFdel{under review}\DIFdelend \DIFaddbegin \DIFadd{accepted for presentation, in press}\DIFaddend .
...
\DIFaddbegin \bibitem{schmid2020}
\DIFadd{Schmid, L.; Pantic, M.; Khanna, R.; Ott, L.; Siegwart, R.; Nieto, J. An Efficient Sampling-Based Method for Online Informative Path Planning in Unknown Environments. \textit{IEEE Robot. Autom. Lett.} \textbf{2020}, \textit{5}, 1500--1507.
}

\bibitem{zhou2021}
\DIFadd{Zhou, B.; Zhang, Y.; Chen, X.; Shen, S. FUEL: Fast UAV Exploration Using Incremental Frontier Structure and Hierarchical Planning. \textit{IEEE Robot. Autom. Lett.} \textbf{2021}, \textit{6}, 779--786.
}

\DIFaddend \bibitem{gonzalez2002}
...
Tarabanis, K.A.; Allen, P.K.; Tsai, R.Y. A survey of sensor planning in computer vision. \textit{IEEE Trans. Robot. Autom.} \textbf{1995}, \textit{11}, 86--104\DIFaddbegin \DIFadd{.
}

\bibitem{zeng2020}
\DIFadd{Zeng, R.; Wen, Y.; Zhao, W.; Liu, Y.-J. View Planning in Robot Active Vision: A Survey of Systems, Algorithms, and Applications. \textit{Comput. Vis. Media} \textbf{2020}, \textit{6}, 225--245.
}

\bibitem{dehbi2021}
\DIFadd{Dehbi, Y.; Leonhardt, J.; Oehrlein, J.; Haunert, J.-H. Optimal Scan Planning with Enforced Network Connectivity for the Acquisition of Three-Dimensional Indoor Models. \textit{ISPRS J. Photogramm. Remote Sens.} \textbf{2021}, \textit{180}, 103--116.
}

\bibitem{knechtel2025}
\DIFadd{Knechtel, J.; Dehbi, Y.; Klingbeil, L.; Haunert, J.-H. Simultaneous Planning of Standpoints and Routing for Laser Scanning of Buildings with Network Redundancy. \textit{ISPRS J. Photogramm. Remote Sens.} \textbf{2025}, \textit{224}, 59--74}\DIFaddend .
