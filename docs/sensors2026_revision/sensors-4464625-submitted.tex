...
\Title{Occlusion-Aware Visibility Coverage for Stop-and-Scan 3D Mapping with a Terrestrial Laser Scanner}
...
growing: a dense, accurate point cloud is becoming an infrastructural asset that
downstream robots localize against, plan collision-free motion within, and reason
over. This raises the practical question of how such maps can be acquired
\emph{automatically} rather than through labor-intensive manual scanning.
...
stationary sensors. A terrestrial laser scanner (TLS) such as the Leica BLK360
G1~\cite{leica-blk360} acquires a dense, colorized point cloud and a panoramic
image from a single viewpoint, with a 3D point accuracy on the order of
millimeters and a full-dome scan time of a few minutes. Two properties of this
device shape the mapping problem. First, the scanner must remain completely
...
aligned offline. Conventional mobile mapping, which assumes continuous
acquisition under motion and incremental online registration, therefore does not
transfer directly to this \emph{stop-and-scan} workflow.

The same workflow is shared by a broad class of high-precision stationary
sensors beyond TLS, including structured-light 3D scanners, thermal panorama
units, ground-penetrating radar, gas and spectral sensors, and non-destructive
testing probes, all of which require either stationary acquisition or a dwell
time at each viewpoint~\cite{stopandgo-review,gas-coverage,robotic-ndt}. Whenever
...
cell within the sensing range as covered. This assumption is reasonable in open
...
In this paper we argue that occlusion-aware visibility, rather than Euclidean
proximity, is the appropriate basis for stop-and-scan placement, and we develop
a coverage model and placement policy built on that principle. We replace the
...
scan pose. The visibility region drives a marginal-gain scan/skip rule that is
...
sensing, with a conservative LOS coverage metric defined over mapped free space
...
model from exploration stochasticity, together with a parameter sensitivity
analysis (Sections~\ref{sec:setup} and~\ref{sec:results});
...
This work extends a preliminary conference manuscript~\cite{iccas2026}, currently
under review at ICCAS 2026, that used an isotropic-disk trigger; here the disk is replaced by the ray-cast visibility
model, the controlled ablation and parameter analysis are new, and a fully
autonomous on-hardware disk-versus-visibility comparison with survey-grade
offline registration of the acquired BLK360 scans is added to complement the
simulation results.
...
shaping~\cite{he2025,quin2021}. Most of these methods, however, assume a sensor
...
surface, and they have been surveyed both for automated 3D reconstruction and
inspection~\cite{scott2003} and for sensor planning in computer
vision~\cite{tarabanis1995}. Coverage path planning, which instead seeks a path
...
Survey-grade terrestrial scanning departs from the continuous-acquisition
assumption: each measurement requires a stationary dwell of minutes and
registration is performed offline~\cite{stopandgo-review}. Prior automated TLS
...
explicitly occlusion- or visibility-aware: Mozaffar and Varshosaz optimize
scanner placement to reduce occlusions~\cite{mozaffar2016occlusion}, and Prieto
et al. drive a next-best-scan policy with a line-of-sight visibility
analysis~\cite{prieto2017nbs}. Crucially, however, these methods operate
\emph{offline on a prior model} of the environment (a BIM, floor plan, or an
initial coarse scan), over which candidate viewpoints and their visibility are
evaluated in advance. The decision of which candidate poses are worth a
...
Let the environment be a two-dimensional occupancy
grid~\cite{elfes1989,thrun2005} $\mathcal{M}$ produced
online by SLAM, following the \texttt{nav\_msgs/OccupancyGrid} convention: each
cell $c$ carries an integer value $o(c)$, where $o(c)=-1$ denotes unmapped
(unknown) space and $o(c)\in[0,100]$ is an occupancy probability. Cells are
classified as
...
with thresholds $\theta_{\mathrm{free}}=25$ and $\theta_{\mathrm{occ}}=65$. The
two thresholds are deliberately separated: a cell is treated as free only when
its occupancy is confidently low ($\le 25$) and as an obstacle only when
confidently high ($\ge 65$); the intermediate band is treated as unknown so that
ambiguous cells are never counted as covered nor allowed to pass a sight line.
We write $\mathcal{F}=\{c\in\mathcal{M}:\mathrm{state}(c)=\textit{free}\}$ for
...
$\mathcal{F}$ is genuinely observed.
...
carries a large fixed cost (a single BLK360 G1 colorized scan takes on the order
of two to three minutes), the placement policy must take as few scans as
possible while still guaranteeing LOS coverage of the free space.
...
$C(S)=\bigcup_i B(s_i,R)$ their union coverage. For a candidate pose $c$, the
decision is based on how much \emph{new} line-of-sight area its visibility region
contributes relative to what it sees in total. We define the \emph{marginal
gain}

\begin{equation}
g(c\mid S)=\frac{\lvert B(c,R)\setminus C(S)\rvert}{\lvert B(c,R)\rvert},
\qquad g\in[0,1],
\end{equation}
...
\begin{equation}
a(c\mid S)=\lvert B(c,R)\setminus C(S)\rvert\cdot\rho^2,
\end{equation}

where $\rho$ is the grid resolution, so $a$ is in m\textsuperscript{2}. Both are
evaluated directly on the visibility regions of Section~\ref{sec:coverage}, so
occlusion is respected: area behind a wall is never part of $B(c,R)$ and never
inflates either the coverage or the gain. A candidate whose visibility region is
degenerate (smaller than $0.5$~m\textsuperscript{2}, which occurs when the pose
projects into an obstacle or an unmapped pocket where almost every ray is
immediately blocked) is rejected outright and raises no scan.
...
of a terrestrial scanner: because $R$ is comparable to the size of a room, the
region $B(c,R)$ is large, so a small but genuinely occluded pocket contributes
...
\begin{equation}
\textsf{scan}(c)\iff
\big(g(c\mid S)\ge\tau\big)\ \lor\ \big(a(c\mid S)\ge A_{\min}\big),
\end{equation}
...
For comparison, the isotropic-disk policy reduces to skipping any candidate
within $R$ (straight-line) of an existing scan, the baseline against which the
visibility rule is evaluated.
...
\While{exploration not converged}{
  ...
  \If{$\lVert \mathrm{pose} - q \rVert_2 \ge d$}{
    $c \leftarrow \mathrm{pose}$;\quad $q \leftarrow c$\;
    $B(c,R) \leftarrow \textsc{RayCastVisibility}(\mathcal{M}, c, R)$\;
    \If{$\lvert B(c,R)\rvert \cdot \rho^2 < 0.5$}{
      \textbf{continue} \tcp*{degenerate pose: skip}
    }
    $g \leftarrow \lvert B(c,R)\setminus C(S)\rvert / \lvert B(c,R)\rvert$\;
    $a \leftarrow \lvert B(c,R)\setminus C(S)\rvert \cdot \rho^2$\;
    \If{$g \ge \tau$ \textbf{\emph{or}} $a \ge A_{\min}$}{
      stop and acquire stationary scan at $c$\;
      $S \leftarrow S \cup \{c\}$\;
    }
  }
}
...
\While{$\exists$ pocket $P \subseteq \mathcal{F}\setminus C(S)$ with $\mathrm{area}(P) \ge A_{\mathrm{pocket}}$ and $P$ reachable}{
  navigate into $P$;\quad acquire scan at reached pose $c$\;
  $S \leftarrow S \cup \{c\}$\;
}
\Return{$S$}\;
\end{algorithm}
...
\caption{System architecture. The frontier exploration layer (left) proposes and
orders navigation goals on the live 2D occupancy grid. The stop-and-scan
sequencer (right) raises a candidate at each distance-gate interval and, using
the occlusion-aware ray-cast visibility region $B(s,R)$ (highlighted), applies
the marginal-gain scan/skip rule; accepted poses yield stationary survey-grade
scans that are registered offline. The visibility coverage model is the
contribution of this work; the exploration layer is an existing component.}
\label{fig:arch}
\end{figure}
...
stationary sensor (Figure~\ref{fig:redbot}). This is the mobile platform whose
absence was noted in the conference version; its availability enables the
real-hardware experiments reported below.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig9_redbot04_platform.jpg}
\caption{RedBOT04, the four-wheeled mobile scan platform used for the
real-hardware experiments, carrying a Leica BLK360 G1 terrestrial laser scanner
(black unit, center) as its survey-grade stationary sensor. The platform
...
pronounced. For the real-hardware experiments (Sections~\ref{sec:realhw}
and~\ref{sec:fieldsim}), a physical
industrial test room is used: an open exhibition and staging space
(Figure~\ref{fig:testroom}) with a flat reflective floor, a display wall along one
side, and freestanding equipment and furniture that create genuine self-occlusion,
representative of the factory and demonstration environments the platform targets.
...
(Section~\ref{sec:fieldsim}) and the per-run offline registration of the
acquired scans (Section~\ref{sec:realhw}).
The open exhibition space has a reflective floor, a
...
We evaluate the visibility-based policy against the isotropic-disk baseline on
two axes, LOS coverage and stationary scan count, organizing the evaluation so
that the most rigorous evidence carries the argument. The simulation study comes
first; Section~\ref{sec:fieldsim} then reports a fully autonomous on-hardware
disk-versus-visibility comparison in a physical industrial test room, with
BLK360 scans acquired live, and Section~\ref{sec:realhw} closes the loop by
...
\caption{Coverage model for a single BLK360 scan pose ($R=6$~m). (\textbf{a})~The
...
\caption{Proposed visibility stop-and-scan on the single-room world ($R=6$~m):
4~scans, $97.9\%$ LOS coverage. The map, robot path, and per-scan visibility
polygons (scan order $\#1\dots\#4$) are overlaid on the Cartographer occupancy
map.}
...
per-path coverage gain reaches $+21$ to $+23$ percentage points. The disk rule is
structurally capped: once two scans are placed, almost every later candidate lies
within $R$ (straight-line) of a prior scan, so it stops scanning even though
walls occlude much of the area it counts as covered.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig5_skip_ablation.png}
\caption{Controlled skip-rule ablation: identical map and identical candidate
path, only the skip decision differs. (\textbf{a},\textbf{b})~single-room and
(\textbf{c},\textbf{d})~multi-room, each under the disk rule
(\textbf{a},\textbf{c}) and the visibility rule (\textbf{b},\textbf{d}). The disk
rule is structurally capped at 2--3 scans and leaves occluded pockets/rooms
($\approx 74$--$87\%$ LOS); the visibility rule fills them ($94$--$95\%$ LOS).}
\label{fig:ablation}
\end{figure}
...
\caption{Three-way skip-rule comparison on identical paths ($N=5$ single-room and
$N=10$ multi-room trajectories, paired per path): uniform no-skip reference,
isotropic-disk baseline, and ray-cast visibility (ours). (\textbf{a})~LOS coverage;
(\textbf{b})~stationary scan count on a log scale (mean\,$\pm$\,s.d.). The
...
the uniform reference's scan count ($28.0$/$40.8$ scans).}
\label{fig:summary}
\end{figure}

\begin{table}[H]
\caption{Controlled skip-rule ablation on identical maps and candidate paths:
three placement policies replayed on the same fixed candidate sequences
($N=5$ single-room paths; $N=10$ multi-room paths, five per coverage model). The
only difference between rows is the coverage model used for the scan/skip
decision. Values are mean\,$\pm$\,s.d. The uniform no-skip policy (scan at every
candidate) is an upper reference: scanning everywhere, it approaches the maximum
LOS attainable along the path but at a prohibitive scan count. The visibility
rule attains LOS within $2.6$--$6.5$ percentage points of that brute-force
reference using only $3.6$ scans on average, while improving on the isotropic
disk baseline by $+8.4$ (single-room) and $+7.0$ (multi-room) percentage points.}
\label{tab:ablation}
...
\midrule
\multirow{3}{*}{Single-room} & Uniform, no skip (reference) & $91.9\pm7.0$ & $28.0\pm13.0$ \\
                             & Isotropic disk (baseline)    & $77.0\pm4.9$ & $2.0\pm0.0$ \\
                             & Ray-cast visibility (ours)   & $\mathbf{85.4\pm6.2}$ & $3.6\pm0.9$ \\
\midrule
\multirow{3}{*}{Multi-room}  & Uniform, no skip (reference) & $86.6\pm9.9$ & $40.8\pm11.8$ \\
                             & Isotropic disk (baseline)    & $77.0\pm6.4$ & $2.3\pm0.5$ \\
                             & Ray-cast visibility (ours)   & $\mathbf{84.0\pm9.2}$ & $3.6\pm0.7$ \\
\bottomrule
\end{tabularx}
\noindent\footnotesize{On paths that traverse every room, the per-path gain of
the visibility rule over the disk rule reaches $+21$ to $+23$ percentage points;
the means above are over all paths including partial-traversal paths. All three
policies traverse the identical candidate path, so total travel distance is the
same across rows; the policies differ only in scan count, the dominant cost in
stop-and-scan sensing.}
\end{table}
...
We characterize the two skip thresholds by deterministic replay over the same
fixed paths as the ablation ($N=5$ single-room, $N=10$ multi-room) per
environment (Figure~\ref{fig:sweep}). Varying the relative
threshold $\tau$ over $[0.10,0.50]$ changes both LOS coverage and scan count by
only about $2$ percentage points, so $\tau$ lies in a robust, flat region and
$\tau=0.30$ is not a sensitive choice. The absolute threshold $A_{\min}$ is the
operative knob: it directly trades coverage against scan count, with its knee at
$A_{\min}=5$~m\textsuperscript{2}, where coverage is within about $5$ percentage
points of the much more aggressive $A_{\min}=1$~m\textsuperscript{2} setting
($90.7\%$) while taking only about half as many scans ($3.6$ versus $7.2$). This
justifies the operating point $\tau=0.30$, $A_{\min}=5$~m\textsuperscript{2}.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig8_param_sweep.png}
\caption{Parameter sweep (deterministic replay, $N=5$ single-room and $N=10$
multi-room paths). (\textbf{a})~$\tau$
sweep at $A_{\min}=5$~m\textsuperscript{2}: coverage and scan count are flat
across $\tau\in[0.10,0.50]$. (\textbf{b})~$A_{\min}$ sweep at $\tau=0.30$:
$A_{\min}$ is the operative coverage-vs-scans knob, with the knee at
$5$~m\textsuperscript{2}.}
\label{fig:sweep}
\end{figure}
...
acquires a full-dome colorized stationary scan (Figure~\ref{fig:field}). Each
...
physical test room (RedBOT04 running frontier exploration $+$ Nav2 $+$
Cartographer live; representative of $N=4$ completed runs), stop-and-scan at
$R=6$~m. Light grey is
mapped free space, dark grey is unmapped, black is occupied; the purple line is
the autonomous robot path and colored markers are the placed scans with their
coverage regions. (\textbf{a})~The disk model places few scans and leaves the
occluded bays uncovered; (\textbf{b})~the visibility model scans them.}
\label{fig:fieldsim}
\end{figure}
...
converged at $0.004$--$0.005$~m with overall overlaps of $84$--$88\%$, and every
individual link fell within the software's highest quality band (maximum error
$\le 0.015$~m; per-link overlaps $74$--$92\%$, errors $0.003$--$0.006$~m). For a
...
floor---as a single coherent colorized model: Figure~\ref{fig:regcloud} shows
...
($75$--$85\%$ overlap, $0.002$~m); the fourth disk run placed only one scan and
...
\begin{table}[H]
\caption{Per-run offline registration of the autonomously acquired BLK360 scans
in Leica Cyclone REGISTER~360 PLUS (BLK Edition), cloud-to-cloud alignment
without targets, identical settings for all runs. Every link of every
registered network lies within the software's highest quality band (maximum
error $\le 0.015$~m).}
\label{tab:registration}
\newcolumntype{Z}{>{\centering\arraybackslash}X}
\begin{tabularx}{\linewidth}{l l Z Z Z Z}
\toprule
\textbf{Policy} & \textbf{Run} & \textbf{Setups} & \textbf{Links} & \textbf{Overall overlap (\%)} & \textbf{Bundle error (m)} \\
\midrule
Visibility & 1 & 3 & 3  & 86 & 0.005 \\
Visibility & 2 & 6 & 13 & 86 & 0.005 \\
Visibility & 3 & 5 & 10 & 84 & 0.005 \\
Visibility & 4 & 3 & 3  & 88 & 0.004 \\
\midrule
...
run~3, rendered along two diagonal viewing directions. (\textbf{Left})~Toward the
display wall with its monitor and product-panel arrays. (\textbf{Right})~Toward
the equipment and workbench side of the room. Red markers and green edges are
the scan stations and cloud-to-cloud links of Table~\ref{tab:registration}; the
...
two additional scans per environment. The effect is structural rather than incidental:
...
Several limitations frame the result. First, LOS coverage is a geometric proxy
...
natural refinement. Second, the controlled paired ablation is conducted in
...
Third, the visibility model presently assumes an optical line-of-sight sensor,
which is
...
We presented an occlusion-aware visibility coverage model for stop-and-scan 3D
mapping with a terrestrial laser scanner, and a marginal-gain placement policy
built on it. Replacing the isotropic disk with a ray-cast visibility region and
scoring candidates by line-of-sight marginal gain corrects the disk model's
structural over-claiming of occluded space. In a physics-based simulation built
...
from about $77\%$ to $84$--$85\%$ at the cost of one to two additional scans, and
a live multi-room run improved coverage from $46.9\%$ to $95.1\%$ relative to the
disk model; an optional coverage-completion phase reached about $99\%$ LOS. A
...
via sensor-level semantic descriptions, and add a downstream navigation validation
in which an independent robot autonomously localizes against the completed map.
