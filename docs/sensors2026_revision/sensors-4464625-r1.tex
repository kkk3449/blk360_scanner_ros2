...
% NOTE: \conference{} hard-codes the prefix "This paper is an extended version of our paper published in ...",
% which is inaccurate while the ICCAS 2026 proceedings are not yet published. The extended-version note is
% therefore set via \firstnote{} with correct wording; restore \conference{} at production once published.
%\conference{the Proceedings of the 2026 International Conference on Control, Automation and Systems (ICCAS), Sapporo, Japan, 27--30 October 2026}
\firstnote{This paper is an extended version of our paper accepted for presentation at the 2026 International Conference on Control, Automation and Systems (ICCAS), Sapporo, Japan, 27--30 October 2026, and to appear in the conference proceedings. The conference version introduced an isotropic-disk stop-and-scan trigger; the present work replaces the disk model with an occlusion-aware ray-cast visibility model, adds a controlled ablation and a parameter sensitivity analysis, and reports a fully autonomous on-hardware disk-versus-visibility comparison on a mobile platform with per-run survey-grade offline registration of the acquired scans.}
...
This work extends a preliminary conference paper~\cite{iccas2026}, accepted
for presentation at ICCAS 2026, and the overlap between the two is as
...
proximity-based: an isotropic-disk or fixed-spacing rule, a commonly used
online treatment in stop-and-go practice, over-claims coverage behind walls and
...
policy must take as few scans as possible while maintaining high LOS coverage
of the mapped free space.

\subsection{Marginal Visibility Gain}

As the robot travels, a scan candidate is raised every $d$ metres of net
straight-line displacement in the map frame from the previous candidate pose
...
path length, so dwelling or circling in place raises no candidate). Let $S=\{s_1,\dots,s_m\}$ be the scans accepted so far and
...
and executes its full-dome scan workflow, and we verified that the device can be
...
Free-space dilation radius   & env.-tuned & Morphological dilation of mapped free space during decision-map construction, before frontier extraction. \\
...
matches the local room/bay scale, and hence the occlusion scale, of the indoor
environments targeted here.
...
only in which candidates they accept (Figure~\ref{fig:ablation}). For the
...
start, are discarded as failed trials rather than policy samples. The exclusion
...
rule. Because each live run maps a slightly different extent of free space,
...
$\le15$~mm; per-link overlaps $74$--$92\%$, errors $3$--$6$~mm). The
$4$--$5$~mm bundle residuals are of the same order of magnitude as the
scanner's stated single-setup 3D point accuracy of a few millimeters.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Patents section removed (no patents resulting from this work).
...
\bibitem{iccas2026}
Kim, S.; Kuc, T.-Y. Coverage-Aware Stop-and-Scan Frontier Exploration for Automated Survey-Grade 3D Mapping. In \textit{Proceedings of the 2026 International Conference on Control, Automation and Systems (ICCAS)}, Sapporo, Japan, 27--30 October 2026; accepted for presentation, in press.
